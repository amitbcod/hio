...
$('.validate-char').on('keypress', function(key) {
    ...
    $('#discount_Coupons_table').DataTable( {
        "order": [[ 1, "desc" ]],
        "info" : false,
        "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]],
        "language": {                
    ...
    if ($('#discount_Coupons_table tr').length < 10) {
...
$('#account_manager').on('submit',function(e){
	e.preventDefault();
if($(this).valid()) {
			var formData = new FormData($("#account_manager")[0]);
				$.ajax({
					type:"POST",

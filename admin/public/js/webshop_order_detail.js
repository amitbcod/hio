...
$(document).ready(function () {
  ...
  // console.log(newSubEndDate);
    ...
    $('#payment_date').datepicker({
      ...
      console.log("Payment date ", converted_date);
      var date_parts = converted_date.split('/');
      var formatted_date = date_parts[2] + '-' + date_parts[1] + '-' + date_parts[0];
      $('#end_date').datepicker('setStartDate', new Date(formatted_date));
...
$("#copy_request_link").click(function (e) {
  ...
  swal({
    title: "",
    text: "Request payment link copied successfully",
    button: false,
    icon: "success",
  });
        ...
        if (obj.status == 200) {
          ...
          if (obj.split_order_id != "" && obj.split_order_id > 0) {
            window.location.href =
              BASE_URL + "webshop/split-order/detail/" + obj.split_order_id;
          } else {
  ...
  if (order_id != "") {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (obj.status == 200) {
          swal("Success", obj.message, "success");
          window.location.href = BASE_URL + "webshop/order/detail/" + order_id;
        } else {
          swal("Error", obj.message, "error");
          return false;
        }
...
function CreateShipmentSave() {
  ...
  var box_ids = $('input[name="box_weight[]"]')
    .map(function () {
      if (this.value > 0) {
        box_val_count++;
      }
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (obj.status == 200) {
          ...
          window.location.href =
            BASE_URL + "webshop/shipped-order/detail/" + obj.order_id;
        } else {
          $("#createShipment").removeAttr("disabled");
          swal("Error", obj.message, "error");
          return false;
  ...
  if (box_ids == "" || box_ids == "[]" || box_val_count < box_count) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (obj.status == 200) {
          window.open(
            BASE_URL + "webshop/order/print-label/" + obj.order_id,
            "_blank"
          );
        } else {
          swal("Error", obj.message, "error");
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (obj.status == 200) {
          window.open(
            BASE_URL +
              "webshop/order/print-label-table/" +
              obj.order_id +
              "/" +
              obj.order_shipment_id,
            "_blank"
          );
        } else {
          swal("Error", obj.message, "error");
...
function ConfirmShipping(order_id) {
  if (order_id != "") {
    ...
    $.ajax({
      ...
      data: {
        order_id: order_id,
        shipping_method_input: shipping_method_input,
      },
      success: function (response) {
        var obj = JSON.parse(response);
        if (obj.status == 200) {
...
$(document)
  .focus()
  .on("keyup", "#conversion_rate", function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.flag == 1) {
          $("#grand_total").val(
            response.data.currency_symbol + " " + response.data.total
          );
        } else {
          return false;
        }
...
function ConfirmCurrencyRating(order_id) {
  if (order_id != "") {
    ...
    $.ajax({
      ...
      data: {
        order_id: order_id,
        currency_option_id: currency_option_id,
        conversion_rate: conversion_rate,
      },
      dataType: "json",
      success: function (response) {
...
function DownloadOrderCSV(order_id) {
  ...
  if (csv_id == 2) {
    download_url =
      "ExportQueryToCsvController/export_order_commercial_invoice_compressed?order_id=" +
      order_id;
    file_name = "export_order_commercial_invoice_compressed";
  } else if (csv_id == 3) {
    download_url =
      "ExportQueryToCsvController/export_order_customer_details?order_id=" +
      order_id;
    file_name = "export_order_customer_details";
  } else {
    download_url =
      "ExportQueryToCsvController/export_order_commercial_invoice_detailed?order_id=" +
      order_id;
...
function InitiateRefund(order_id) {
  if (order_id != "") {
    $.ajax({
      url: BASE_URL + "WebshopOrdersController/InitiateRefundPopup",
      type: "POST",
      data: {
        order_id: order_id,
      },
...
$(document).on("submit", "#initiate_refund_form", function (event) {
  ...
  var form = $("#initiate_refund_form");
  $.ajax({
    url: BASE_URL + "WebshopOrdersController/InitiateRefund",
    type: "POST",
    datatype: "json",
    data: form.serialize(),
    success: function (response) {

	...
	public function _get_datatables_query_orders($term, $price, $order_status, $shipment_type, $fromDate, $toDate, $payment_method)
{
	...
	if ($current_tab == 'pickup-orders') {
		// Join pickup details if exists, but don't exclude orders without pickup
		$this->db->join('b2b_orders_pickup_details as pod', 'o.order_id = pod.order_id', 'LEFT');

		// Include orders whose pickup is completed or allow NULL (no pickup)
		$this->db->group_start();
			$this->db->where('pod.pickup_status !=', 4);
			$this->db->or_where('pod.pickup_status IS NULL', null, false); // include orders with no pickup yet
		$this->db->group_end();

		// Delivery related statuses
		$this->db->where_in('o.status', [10,11]);
	}
    ...
    // Include orders whose pickup is completed or allow NULL (no pickup)
    $this->db->group_start();
        $this->db->where('pod.pickup_status', 4); // delivered to warehouse
        $this->db->or_where('pod.pickup_status IS NULL', null, false); // include orders with no pickup yet
    $this->db->group_end();

    // Delivery related statuses
    $this->db->where_in('o.status', [4, 5, 6, 7,10,12,13]);
}
    ...
    // Only exclude status 7 for other tabs, not delivery-orders
	...
	public function get_datatables_delivery_orders($price, $order_status, $shipment_type, $fromDate, $toDate, $payment_method)
{
    $term = isset($_REQUEST['search']['value']) ? $_REQUEST['search']['value'] : '';

    $this->db->distinct();
    $this->db->select('o.*, IF(o.shipment_type=1,p.publication_name,CONCAT(o.customer_firstname, " ", o.customer_lastname)) as customer_name');
    $this->db->from('b2b_orders as o');
    $this->db->join('publisher as p', 'o.publisher_id = p.id', 'LEFT');
    $this->db->join('sales_order_address as soa', 'soa.order_id = o.webshop_order_id', 'LEFT');
    $this->db->join('b2b_orders_pickup_details as pod', 'o.order_id = pod.order_id', 'LEFT');
    ...
    // Only delivery-related statuses
    $this->db->where_in('o.status', [4, 5, 6, 12]);

    // Search filter
    if ($term != '') {
        $this->db->group_start();
        $this->db->like('o.increment_id', $term);
        $this->db->or_like('o.grand_total', $term);
        $this->db->or_like('o.customer_firstname', $term);
        $this->db->or_like('o.customer_lastname', $term);
        $this->db->or_like('p.publication_name', $term);
        $this->db->or_like('soa.city', $term);
        $this->db->group_end();
    }
    ...
    if (!empty($shipment_type)) {
        $this->db->where_in('o.shipment_type', explode(',', $shipment_type));
    }
    if (!empty($payment_method)) {
    ...
    if (isset($_REQUEST['length']) && $_REQUEST['length'] != -1) {
        ...
        $this->db->limit($_REQUEST['length'], $start);
    }

    return $this->db->get()->result();
}
public function count_all_delivery_orders($price, $order_status, $shipment_type, $fromDate, $toDate, $payment_method)
{
    $term = (isset($_REQUEST['search']['value']) && $_REQUEST['search']['value'] != '') 
    ...
    // Exclude orders that are still "To Be Processed"
    $this->db->where('o.status !=', 0); 
    $this->db->where('o.status !=', 10); 

    // Include only delivery related statuses
    $this->db->where_in('o.status', [4, 5, 6, 12]);
    ...
    // Exclude status 7 (for delivery-orders it’s already filtered)
    // Ordering
    $column = array('o.increment_id', 'o.created_at', 'customer_name', '', '', '', '');
    if (isset($_REQUEST['order'])) {
        $this->db->order_by($column[$_REQUEST['order']['0']['column']], $_REQUEST['order']['0']['dir']);
...
public function getAllB2COrders()
{
    $this->db->select('*');
    $this->db->from('sales_order');
    $this->db->where('parent_id', 0); // only parent B2C orders
    $this->db->where('status !=', 2); // exclude records where status = 2
    $this->db->order_by('created_at', 'DESC');
    $query = $this->db->get();
    return $query->result();
}

public function getAllcompleteB2COrders()
{
    $this->db->select('*');
    $this->db->from('sales_order');
    $this->db->where('parent_id', 0); // only parent B2C orders
...
public function getSubOrdersByWebshopOrderId($webshop_order_id)
{
    return $this->db

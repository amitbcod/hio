...
function get_datatables_products($price, $inventory, $supplier, $fromDate, $toDate, $count, $image_filter, $approval_status = null)
{
    $term = isset($_REQUEST['search']['value']) ? $_REQUEST['search']['value'] : '';
    
    // Build query with existing filters
    $this->_get_datatables_query_products($term, $price, $inventory, $supplier, $fromDate, $toDate, $image_filter);
    
    // Apply approval_status filter if provided
    if (!is_null($approval_status) && $approval_status !== '') {
        $this->db->where('p.approval_status', intval($approval_status));
    }
	...
	function get_datatables_products_all_count($price, $inventory, $supplier, $fromDate, $toDate, $image_filter, $approval_status = null)
	{
		$term = isset($_REQUEST['search']['value']) ? $_REQUEST['search']['value'] : '';
		
		$this->_get_datatables_query_products($term, $price, $inventory, $supplier, $fromDate, $toDate, $image_filter, $approval_status);
	...
	public function _get_datatables_query_products($term, $price, $inventory, $supplier, $fromDate, $toDate, $image_filter, $approval_status = null)
	{
		...
		if ($term != '') {
			$check_term = $this->CommonModel->custom_filter_input($term);
			$this->db->where(" (
				p.name LIKE '%" . $check_term . "%'
				OR p.product_code LIKE '%" . $check_term . "%'
				OR p.sku LIKE '%" . $check_term . "%'
				OR p.barcode LIKE '%" . $check_term . "%'
				OR p.id IN (select `parent_id` FROM `products` WHERE `product_type` = 'conf-simple' AND (`barcode` LIKE '%" . $check_term . "%' OR `sku` LIKE '%" . $check_term . "%'))
			)");
		}
		...
		if (!empty($supplier)) {
			$selected_supplier = strpos($supplier, ',') !== false ? explode(',', $supplier) : [$supplier];
		...
		if (!empty($fromDate)) $this->db->where('p.updated_at >=', strtotime($fromDate));
		if (!empty($toDate)) $this->db->where('p.updated_at <=', strtotime($toDate));
	...
	// 	$Response['webshop_price'] = round($webshop_price);
	...
	function get_publication_by_id($id)
	{
		...
		if ($resultArr) {
			...
			return null; // or 0 if you prefer a numeric default
		}
	}

<div class="barcode-qty-box row order-details-sec-top">
	<div class="col-sm-6 order-id">
		...
		<p><span>Order Status :</span> 
			<?php  
				// echo $OrderPaymentDetail->payment_method_name;die;
				if($OrderPaymentDetail->payment_method_name == "Cheque/Funds Transfer"){
				?>Pending
				<?php }else {?>
					<?php echo $this->CommonModel->getOrderStatusLabel($OrderData->status); ?>
				<?php  }?>
			<?php if ($OrderData->status==3 && $OrderData->cancel_refund_type ==2) { echo '( Refund Done )</p><p> <span>Refund Note :</span>'.$OrderData->refund_note; } ?>
		</p>
		...
		<p class="ship-ad-para"><span>Billing Address :</span> <span class="order-address-inner"><?php
																									...
																									if (isset($BillingAddress) && $BillingAddress->address_id != '') {
																										...
																										if ($billMobile) {
																											echo '<br/>Mob:';
																											echo $billMobile;
																											print_r($OrderData->company_name);
																										}
		...
		<div class="col-sm-4 order-id">
			<?php if (isset($OrderData->parent_id) && $OrderData->parent_id > 0) { ?>
				...
				<?php if (isset($shop_data) && $shop_data->vat_flag == 1) { ?>
					<p class="position-relative"><span> Shipping Method : </span><?php if ($ParentOrder->ship_method_name == "") {
																						echo "-";
																					} else {
																						echo $ParentOrder->ship_method_name;
																					}  ?><?php if ($ParentOrder->status == 0) { ?><button type="button" class="btn btn-primary change-add-n-btn" onclick="OpenShippingPopup(<?php echo $ParentOrder->order_id; ?>);" style="right:0px;"><i class="fas fa-edit"></i></button><?php } ?></p>
				<?php } ?>

				<?php if (isset($ParentOrder->voucher_code) && ($ParentOrder->voucher_code != '' && $ParentOrder->voucher_amount > 0)) { ?>
					<p><span> Gift Card Amount (<?php echo $ParentOrder->voucher_code; ?>) :</span> <?php echo $currency_code . ' ' . number_format($ParentOrder->voucher_amount, 2); ?></p>
				<?php } ?>
			...
			<?php } else { ?>
				...
				<?php if (isset($shop_data) && $shop_data->vat_flag == 1) { ?>
					<p class="position-relative"><span> Shipping Method : </span><?php if ($OrderData->ship_method_name == "") {
																						echo "-";
																					} else {
																						echo $OrderData->ship_method_name;
																					}  ?><?php if ($OrderData->status == 0) { ?><button type="button" class="btn btn-primary change-add-n-btn" onclick="OpenShippingPopup(<?php echo $OrderData->order_id; ?>);" style="right: 0px;"><i class="fas fa-edit"></i></button><?php } ?></p>
				<?php } ?>

				<?php if (isset($OrderData->voucher_code) && ($OrderData->voucher_code != '' && $OrderData->voucher_amount > 0)) { ?>
					<p><span> Gift Card Amount (<?php echo $OrderData->voucher_code; ?>) :</span> <?php echo $currency_code . ' ' . number_format($OrderData->voucher_amount, 2); ?></p>
				<?php } ?>
		...
		<div class="col-sm-4 order-id">
			<p class="position-relative"><span>Payment Mode :</span>
				...
				if ($OrderData->voucher_code != '' && $OrderData->grand_total <= 0) {
					$payment_method_name = 'Gift Card Payment';
				} else {
			...
			<?php if ($this->session->userdata('ShopID') === '1' && !empty($OrderPaymentDetail)) { ?>
				...
				<table style="width: 100%">
					<tr>
						...
						<td><?= $OrderPaymentDetail->status ?? '' ?></td>
					</tr>
					<tr>
						<th>Payment Amount</th>
						<td><?= $OrderPaymentDetail->payment_amount ?? '' ?> <?= $OrderPaymentDetail->payment_currency ?? '' ?></td>
					</tr>
					<?php if (($OrderPaymentDetail->payment_currency ?? '') !== ($OrderPaymentDetail->currency_code ?? '')) : ?>
						<tr>
							<th>Converted Payment Amount</th>
							<td><?= $OrderPaymentDetail->amount ?? '' ?> <?= $OrderPaymentDetail->currency_code ?? '' ?></td>
						</tr>
					<?php endif; ?>
					<tr>
					...
					if (!empty($OrderRefundDetail)) :
						foreach ($OrderRefundDetail->result() as $orderRefund) : ?>
							<tr>
								<th>Payment Refund <?= $orderRefund->id ?>:</th>
								<td>

      ...
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3">
		  ...
		  <!-- form -->
    		...
    		<div class="parent-menu-list" id="category_menu_list" style="">
         	<div class="row">
   <div class="col-md-6">
      <h1 class="head-name">Attributes Translation</h1>
   </div>
...
<?php if(isset($AttributesList) && !empty($AttributesList)) {?>
<form method="POST" id="categoryMenuForm" action="">
   <input type="hidden" name="block_id" value="">
   <div class="parent-menu-list-inner">
      <ul>
         <?php foreach($AttributesList as $main_cat) {
            $id = $main_cat['id'];
            ?> 
         <li>
            <div class="row">
               ...
               <div class="col-md-6 edit-add-translation">
                  <?php foreach ($languagesListing as $key => $value) { 
                     ...
                     {
                      ?>
                  <a class="edit-cat fa fa-edit"  title="<?php echo $value['name']; ?>" onclick="OpenEditAttributes(<?php echo $main_cat['id']; ?>,'<?php echo $value['code']; ?>');"></a> 
                  <?php } else {   ?>
                  <a class="add-cat fa fa-plus"  title="<?php echo $value['name']; ?>" onclick="OpenEditAttributes(<?php echo $main_cat['id']; ?>,'<?php echo $value['code']; ?>');"></a>
                  <?php } } ?> 
               </div>
            </div>
         </li>

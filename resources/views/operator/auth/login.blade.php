...
@section('content')
    <style>
        ...
        .btn-primary {
            /* background: linear-gradient(90deg, #F59224 0%, #73BD44 100%); */
            background: #41b0aa;
            border: none;
        }
            ...
            <div class="login-form-section">
                ...
                <form method="POST" action="{{ route('operator.login') }}">
                    ...
                    @if(request('accept_token') || session('accept_token'))
                        <input type="hidden" name="accept_token"
                            value="{{ request('accept_token') ?? session('accept_token') }}">
                    @endif
                    ...
                    <button type="submit" class="btn btn-primary w-100 mb-2 fw-semibold">Sign In</button>
                    @if($errors->any())

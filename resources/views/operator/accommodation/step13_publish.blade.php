        ...
        <div class="row">
            ...
            <div class="col-md-9">
                ...
                <div style="background:#fff;border-radius:16px;padding:24px;box-shadow:0 2px 16px rgba(0,0,0,0.07);margin-bottom:24px;">
                    <div style="display:flex;justify-content:space-between;align-items:start;">
                        <div>
                            ...
                            <p style="margin:0;font-size:14px;color:#666;">{{ $accommodation->property_name }}</p>
                        </div>
                        <div style="text-align:right;">
                            <p style="margin:0;font-size:12px;color:#999;">Accommodation ID: {{ $accommodation->id }}</p>
                            @if($accommodation->approval_status)
                                <span style="display:inline-block;margin-top:8px;padding:6px 12px;border-radius:20px;font-size:12px;font-weight:600;
                                    background:{{ $accommodation->approval_status === 'Approved' ? '#e8f5e9' : ($accommodation->approval_status === 'Pending' ? '#fff3e0' : ($accommodation->approval_status === 'Rejected' ? '#ffebee' : '#f5f5f5')) }};
                                    color:{{ $accommodation->approval_status === 'Approved' ? '#2e7d32' : ($accommodation->approval_status === 'Pending' ? '#f57c00' : ($accommodation->approval_status === 'Rejected' ? '#c62828' : '#666')) }};">
                                    {{ $accommodation->approval_status }}
                                </span>
                            @endif
                        </div>
                    </div>
                ...
                <div style="background:#fff;border-radius:12px;padding:32px;box-shadow:0 2px 12px rgba(0,0,0,0.04);text-align:center;">
                    @if($accommodation->approval_status === 'Approved')
                        <!-- Approved State -->
                        <div style="max-width:500px;margin:0 auto;">
                            <div style="font-size:48px;margin-bottom:16px;">✅</div>
                            <h3 style="color:#2e7d32;font-weight:600;margin-bottom:12px;">Accommodation Approved!</h3>
                            <p style="color:#666;margin-bottom:8px;">Your accommodation has been approved and is now live.</p>
                            <p style="font-size:13px;color:#999;">Approved on: {{ $accommodation->approved_at ? $accommodation->approved_at->format('M d, Y h:i A') : 'N/A' }}</p>
                            @if($accommodation->approval_notes)
                                <div style="background:#e8f5e9;border-left:4px solid #28a745;padding:16px;margin-top:20px;text-align:left;border-radius:4px;">
                                    ...
                                    <p style="margin:8px 0 0 0;color:#555;font-size:13px;">{{ $accommodation->approval_notes }}</p>
                                </div>
                            @endif
                        </div>
                    @elseif($accommodation->approval_status === 'Pending')
                        <!-- Pending State -->
                        <div style="max-width:500px;margin:0 auto;">
                            <div style="font-size:48px;margin-bottom:16px;">⏳</div>
                            <h3 style="color:#f57c00;font-weight:600;margin-bottom:12px;">Pending Approval</h3>
                            <p style="color:#666;margin-bottom:8px;">Your accommodation has been submitted and is awaiting admin approval.</p>
                            <p style="font-size:13px;color:#999;">Submitted on: {{ $accommodation->submitted_for_approval_at ? $accommodation->submitted_for_approval_at->format('M d, Y h:i A') : 'N/A' }}</p>
                    ...
                    @elseif($accommodation->approval_status === 'Rejected')
                        <!-- Rejected State -->
                        <div style="max-width:500px;margin:0 auto;">
                            <div style="font-size:48px;margin-bottom:16px;">❌</div>
                            ...
                            @endif
                            <form method="POST" action="{{ route('operator.accommodation.submit-approval', $accommodation->id) }}">
                                @csrf
                    ...
                    @else
                        ...
                        <div style="max-width:500px;margin:0 auto;">
                            ...
                            <p style="color:#666;margin-bottom:24px;">You've completed all the required steps. Submit your accommodation for admin approval to make it live on the platform.</p>
                            ...
                            <div style="background:#e3f2fd;border-left:4px solid #2196f3;padding:16px;margin-bottom:24px;text-align:left;border-radius:4px;">
                                ...
                                <ul style="margin:12px 0 0 0;padding-left:20px;color:#555;font-size:13px;line-height:1.8;">
                                    ...
                                    <li>Room rates and inventory are configured</li>
                            ...
                            <form method="POST" action="{{ route('operator.accommodation.submit-approval', $accommodation->id) }}">
                                ...
                                <p style="margin-top:16px;font-size:12px;color:#999;">Your accommodation will be reviewed by the admin team</p>
                            </form>

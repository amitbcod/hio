...
@php $sidebar = 'admin.groups._steps_sidebar'; $currentStep = 2; @endphp

@section('content')
<div class="container mt-5">
    <div style="background:#fff;border-radius:12px;padding:18px;margin-bottom:16px;">
        <h1 style="margin:0;font-weight:700;">Add New Group</h1>
        <p style="color:#666;margin-top:6px;">Build the day-by-day itinerary for this group</p>
    </div>

    <div style="background:#fff;border-radius:12px;padding:20px;box-shadow:0 2px 12px rgba(0,0,0,0.04);">
        <h5 style="background:#0e8f84;color:#fff;padding:10px;border-radius:6px;display:inline-block;">Step 2: Add Group</h5>

        @php $itinerary = $group->itinerary ?? []; @endphp
        <form method="POST" action="{{ route('admin.groups.step2.store', $group->id) }}">
            @csrf
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="itinerary[{{ $i }}][activity]" class="form-select activity-select" data-day-index="{{ $i }}">
                                ...
                                @foreach($availableActivities[$i] ?? [] as $actEntry)
                                    @php
                                        $act = $actEntry['model'];
                                        $hasPkg = !empty($actEntry['has_package']) ? 1 : 0;
                                    @endphp
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;margin-top:12px;">
                <a href="{{ route('admin.groups.edit', $group->id) }}" class="btn btn-light">Back</a>
                <button type="submit" class="btn btn-primary next-prefix">Next: Group Allocation</button>
            </div>
        </form>
    </div>

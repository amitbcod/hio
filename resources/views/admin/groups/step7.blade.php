...
@php $sidebar = 'admin.groups._steps_sidebar'; $currentStep = 7; @endphp
...
<div class="container mt-4 mb-5">
    <div class="card border-0 shadow-sm mb-4" style="border-radius:14px; overflow:hidden;">
        <div class="card-body p-4">
            ...
            <p class="mb-0 text-muted">Effective group policy summary across the selected accommodations/operators for this group.</p>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm" style="border-radius:14px; overflow:hidden;">
        <div class="card-body p-4">
            ...
            <div style="border:1px solid #e4e7eb;border-radius:10px;overflow:hidden;background:#fff;">
                <form method="POST" action="{{ route('admin.groups.step7.save', $group->id) }}">
                    @csrf
            ...
            <div style="margin-top:16px;border:1px solid #e4e7eb;border-radius:10px;padding:12px 14px;background:#f7f7f7;">
                ...
                <textarea name="group_notes" class="form-control" rows="3">{{ $effectivePolicy['group_notes'] ?? '' }}</textarea>
            </div>

            <div class="d-flex justify-content-between mt-4 align-items-center">
                <a href="{{ route('admin.groups.step6', $group->id) }}" class="btn btn-outline-secondary">Back</a>

                <div class="d-flex gap-2">
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-outline-primary" name="action" value="draft">Save as Draft</button>
                        <button type="submit" class="btn btn-primary" style="flex:1" name="action" value="published" onclick="return confirm('Are you sure you want to publish this group?')">Save and Publish</button>
                    </div>
                    </form>
                </div>
            </div>

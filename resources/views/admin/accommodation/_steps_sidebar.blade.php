...
<div style="">
    ...
    <div style="display:flex;flex-direction:column;gap:0px;">
        ...
        @foreach($steps as $stepNum => $stepData)
            ...
            @endphp
            @if($accommodation)
                <a href="{{ route($routeName, $accommodation->id) }}" style="padding:10px 12px;background:{{ $isActive ? '#1e5f83' : ($isComplete ? '#154f6eff' : 'transparent') }};border-left:0px solid {{ $isActive ? '#2196f3' : ($isComplete ? '#28a745' : '#ccc') }};border-radius:0px;text-decoration:none;font-size:13px;color:{{ $isActive ? '#9ddcff' : ($isComplete ? '#28a745' : '#ffffff') }};font-weight:{{ $isActive ? '600' : '500' }};display:flex;justify-content:space-between;align-items:center;border-bottom: 1px solid #477993;">

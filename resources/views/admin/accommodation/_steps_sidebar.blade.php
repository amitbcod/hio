        ...
        @foreach($steps as $stepNum => $stepData)
            ...
            @else
                <div style="padding:10px 12px;background:transparent;border-left:0px solid #ccc;border-radius:0px;font-size:13px;color:#ffffff;font-weight:500;display:flex;justify-content:space-between;align-items:center;border-bottom: 1px solid #477993;">
                    <span>Step {{ $stepNum }}: {{ $stepData['name'] }}</span>
                    <span style="font-size:12px;background: #557f97; border-radius: 30px; width: 20px; height: 20px; align-items: center; display: flex; justify-content: center; font-weight: bold;">{{ $stepNum }}</span>
                </div>
            @endif
        @endforeach
    </div>

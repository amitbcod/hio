...
@section('content')
                            ...
                            <div class="transport-row">
                                <div class="transport-field" style="flex: 0 1 210px; min-width: 210px;">
                                    ...
                                    <select name="service_type" class="category-search-select">
                                        ...
                                        <option value="hotel_transfer" {{ ($filters['service_type'] ?? '') === 'hotel_transfer' ? 'selected' : '' }}>Hotel Transfer</option>
                                        <option value="full_day_sightseeing" {{ ($filters['service_type'] ?? '') === 'full_day_sightseeing' ? 'selected' : '' }}>Full Day Sightseeing</option>
                                        <option value="half_day_sightseeing" {{ ($filters['service_type'] ?? '') === 'half_day_sightseeing' ? 'selected' : '' }}>Half Day Sightseeing</option>
                                    </select>
                                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const buildTimePicker = function(wrapper) {
                const input = wrapper.querySelector('input[type="hidden"], input[type="time"]');
                ...
                clickableTarget.addEventListener('pointerdown', onDisplayClick);
            ...
            const bindTimePickers = function() {
                document.querySelectorAll('.custom-picker-wrapper.time-picker').forEach(buildTimePicker);
            };

            // Global click handler to open timepicker when click coordinates fall inside wrapper
    ...
    @push('styles')
        <style>
            ...
            .custom-timepicker-ok { border: 0; background: #0a84ff; color: #fff; padding: 7px 12px; border-radius: 6px; cursor: pointer; }</style>
    @endpush
                to: null,
            };

...
return new class extends Migration
{
    public function up()
    {
        Schema::table('operator_profiles', function (Blueprint $table) {
            ...
            $table->index('business_id', 'idx_operator_profiles_business_id');
            $table->foreign('business_id')->references('id')->on('businesses')->onDelete('set null');
        });

        // No backfill performed — existing data are test data and will be ignored per project decision.
        // Future data will use the new `business_id` column. (No destructive migrations or data copy executed.)
    }
    ...
    public function down()
    {
        Schema::table('operator_profiles', function (Blueprint $table) {
            ...
            $table->dropIndex('idx_operator_profiles_business_id');

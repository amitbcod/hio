$(document).ready(function(){
	...
	CheckState($('#billing_country').val(),billing_state_div,billing_dp_state_div,billing_state_dp,billing_state);
	...
	CheckState($('#shipping_country').val(),shipping_state_div,shipping_dp_state_div,shipping_state_dp,shipping_state);
	...
	CheckAddressValMax($('#billing_country').val(),'#billing_address','#billing_address_1');

	$("#billing_country").change(function() {
		CheckAddressValMax($('#billing_country').val(),'#billing_address','#billing_address_1');
	});

	CheckAddressValMax($('#shipping_country').val(),'#shipping_address','#shipping_address_1');
	...
	$("#order-frm").validate({
		...
		errorPlacement: function(label, element) {
			...
			} else if(element.attr("name") == "shipping_method") {
				label.appendTo("#check_shipMethod_error");
			} else {
	...
	$.validator.addMethod("ValMaxCharCheck_billing", function(value, element) {
		var country = $("#billing_country").val();
	...
	$.validator.addMethod("ValMaxCharCheck_shipping", function(value, element) {
		var country = $("#shipping_country").val();
		if ((country == 'IN' && value.length > 150) || (country == '' && (SHOP_FLAG == 2 || SHOP_FLAG == 1 || SHOP_FLAG == 4) && value.length > 150)) {
			address_validation_mssg = 'Please enter no more than 150 characters.';
			return false;
	...
	$.validator.addMethod("CheckPinCodeBill", function(value, element) {
		var country = $("#billing_country").val();
		return checkPinBaseOnCountry(value,country);
	},"Invalid Pin code");

	$.validator.addMethod("CheckPinCodeShip", function(value, element) {
		var country = $("#shipping_country").val();
		return checkPinBaseOnCountry(value,country);
	},"Invalid Pin code");
	...
	$.validator.addMethod("Pin_noCheck", function(value, element) {
		var country = $("#billing_country").val();
		result = changeCountryBasedOnPostalCode(value,country,'#billing_country');
		return true;
	},"pin");

	$.validator.addMethod("Pin_noCheckShipping_Country", function(value, element) {
		var country = $("#shipping_country").val();
		result = changeCountryBasedOnPostalCode(value,country,'#shipping_country');
		return true;
	...
	$.validator.addMethod("vat_noCheck",function(val, elem){
		...
		if( event.target.id == 'billing-address-save' || event.target.id == 'vat_no') {

			if(event.type == "click" || event.type == "submit" ||  event.type == "blur" ) {
					...
					if(response.flag == 0){
						...
						if(final_res.response_type == 0){
									...
									success:function(response){
										...
										if (response.flag == 0) {
											if(response.validitiy == 'Valid'){
												...
												$('#vatFlag').val("1");
												valid = true;
												var billing_address_options=$("input[name='billing_address_options']:checked").val();

												if(billing_address_options == 'new'){
													$( '<label id="vat_no-success" class="success-msg" for="vat_no"> '+ vat_no + ' is valid. </label>' ).insertAfter( ".loaderDiv" );
													$(".loaderDiv").hide();
												} else {
												...
												if(billing_address_options == 'new'){
													$( '<label id="vat_no-error" class="error-msg" for="vat_no"> '+ vat_no + ' is not valid. Vat will be charged.</label>' ).insertAfter( ".loaderDiv" );
												} else {
													swal('Error',val+" is not valid. Vat will be charged",'warning');
												}
											}
										} else {
											...
											if(billing_address_options == 'new'){
												$( '<label id="vat_no-error" class="error-msg" for="vat_no"> '+ response.msg + '. Vat will be charged.</label>' ).insertAfter( ".loaderDiv" );
												$(".loaderDiv").hide();
											} else {
												swal('Error', response.msg+". Vat will be charged",'warning');
											}
										}
									}
								});
							...
							if(billing_address_options == 'new'){
								...
								swal('Error',vat_no+" is not valid. Vat will be charged",'warning');
							}
	...
	$('#billing-address-save').click(function(event) {
		...
		if(form.valid() === true)
		{
			...
			if(same_as_billing==1){
				...
				var billing_address_options=$("input[name='billing_address_options']:checked").val();
				$('#temp_bill_country').val($('input[name="billing_address_options"]:checked').data('country'));
				...
				if(allowed_ship_country!=''){
					...
					if(billing_address_options=='new'){
						...
						if(checkCountryExist==false){
							swal('Error','Shipping does not allowed to this country, Please select proper country.','error');
							return false;
						}
					} else {
						var flag_allowed = CheckBillingShippingCountry(billing_address_options);
				...
				if(billing_address_options=='new'){
					...
					$("input[name=address_options][value=" + billing_address_options + "]").attr('checked', 'checked');
				...
				$('#shipping-tab').attr('disabled',true);
				$('#collapseTwo').removeClass('show');
			...
			$('#collapseShippingMethod').removeClass('show');
			return false;
		}
	...
	$('#address-save').click(function(event) {
		...
		if(form.valid() === true)
		{
			...
			$('#shipping-tab').attr('disabled',false);
			$('#collapseTwoNew').removeClass('show');
			$('#collapseShippingMethod').addClass('show');
		} else {
			$('#shipping-tab').attr('disabled',true);
			$('#place_order').attr('disabled',true);
			$('#collapseTwoNew').addClass('show');
	...
	$('#shipping-method-save').click(function(event) {
...
function updateShippingMethods(eu_shipping_response) {
	let checkSelectedShippingMethod = $('#checkout-shipping-method input[name=shipping_method]:checked').val();
	if(checkSelectedShippingMethod !== undefined){
		selectedShippingMethod = checkSelectedShippingMethod;
	}

	$("#checkout-shipping-method").html(eu_shipping_response);

	if(selectedShippingMethod !== undefined) {
		checkSelectedShippingMethod = $('#checkout-shipping-method input[name=shipping_method]:checked').val();
		if(checkSelectedShippingMethod === undefined) {
			$('#checkout-shipping-method input[name=shipping_method][value='+selectedShippingMethod+']').attr('checked', 'checked');
		}
	}
	...
	$.ajax({
		...
		success: function(response){
			if(response.flag == 0 && response.msg == 'Success'){
				...
				if(response.eu_shipping_response !=  false || response.eu_shipping_response !=  '' ){
					updateShippingMethods(response.eu_shipping_response);


					if(response.shipping_response_flag == 0){
						$("#shipping-method-save").hide();
					} else {
						$("#shipping-method-save").show();
						$("#shipping-method-save").removeClass('disabled_payment_continue');
						$("#shipping-method-save").prop('disabled', false);
					}
				}
...
function SaveQuoteAddress(address_type){
	updateShippingMethods("Loading shipping options");
	$("#shipping-method-save").addClass('disabled_payment_continue');
	$("#shipping-method-save").prop('disabled', true);
	...
	if(address_type==1){
		...
		if(billing_country == 'IN') {
			var billing_state=$('#b_state_dp').val();
		} else {
		...
		var save_in_address_book=$("input[name='billing_save_in_address_book']:checked").val();

		$.ajax({
			url: BASE_URL+'CheckoutController/saveQuoteAddress',
			type: 'POST',
			dataType:'json',
			data: {billing_first_name:billing_first_name,billing_last_name:billing_last_name,billing_address:billing_address,billing_address_1:billing_address_1,billing_email_id:billing_email_id,billing_mobile_no:billing_mobile_no,billing_country:billing_country,billing_pincode:billing_pincode,billing_state:billing_state,billing_city:billing_city,save_in_address_book:save_in_address_book,billing_address_options:billing_address_options,address_type:address_type,company_name:company_name,vat_no:vat_no,consulation_no:consulation_no,res_company_name:res_company_name,res_company_address:res_company_address,vat_vies_valid_flag:vat_vies_valid_flag},
			...
			success: function(response){
				if (response.flag == 1) {
					...
					if(response.shipping_response_flag === 1) {
						updateShippingMethods(response.eu_shipping_response);
					}
		...
		var save_in_address_book=$("input[name='save_in_address_book']:checked").val();

		$.ajax({
			url: BASE_URL+'CheckoutController/saveQuoteAddress',
			type: 'POST',
			dataType:'json',
			data: {shipping_first_name:shipping_first_name,shipping_last_name:shipping_last_name,shipping_address:shipping_address,shipping_address_1:shipping_address_1,shipping_mobile_no:shipping_mobile_no,shipping_country:shipping_country,shipping_pincode:shipping_pincode,shipping_state:shipping_state,shipping_city:shipping_city,save_in_address_book:save_in_address_book,address_options:address_options,address_type:address_type,same_as_billing:same_as_billing},
			...
			success: function(response){
				...
				if (response.flag == 1) {
					...
					ChangeVatSession(response.country_code);
...
function SaveShippingMethod(){
	var shipping_charge_id=$("input[name='shipping_method']:checked").val();

	$.ajax({
		url: BASE_URL+'CheckoutController/saveShippingMethod',
		type: 'POST',
		dataType:'json',
...
function RefreshPaymentMethods(){
	$('<span class="spinner-border spinner-border-sm" id="payment-loader" role="status" aria-hidden="true">Please Wait...</span>').insertBefore("#payment-save");
...
function SameAsBillingAddress(address_type){
	if(address_type=='new'){
		...
		CheckState($('#billing_country').val(),shipping_state_div,shipping_dp_state_div,shipping_state_dp,shipping_state,1,billing_state_dp,billing_state);
...
function ValidOTP(){
	...
	if(otp_password == '')
	{
	  	...
	  	document.getElementById('otp_password').style.border="1px solid #eb340a !important";
	  	document.getElementById('otp_password').style.background="#faebe7 !important";
	  	document.getElementById('error-msg').innerHTML='This is a required field.';
		...
		jQuery("#Loader").hide();
	  	...
	  	document.getElementById('otp_password').style.border="1px solid #eb340a !important";
	  	document.getElementById('otp_password').style.background="#faebe7 !important";
	  	document.getElementById('error-msg').innerHTML='Please enter numbers only.';
		$("#valid-otp-btn").removeAttr('disabled');
	  	return false;
		...
		$.ajax({
			...
			type: 'POST',
			dataType:'json',
			data: {order_id:order_id,otp_password:otp_password,phone_no:phone_no},
			success: function(response){
				if (response.flag == 1) {
	  	...
	  	document.getElementById('otp_password').style.border="1px solid #eb340a !important";
	  	document.getElementById('otp_password').style.background="#faebe7 !important";
	  	document.getElementById('error-msg').innerHTML='Please enter valid OTP password.';
		$("#valid-otp-btn").removeAttr('disabled');
	  	return false;
	}
}
...
function addingVatLog(request,response,response_type,address_id=''){
	...
	$.ajax({
		...
		method: 'POST',
		dataType: 'json',
		data: {request:request,response:response,response_type:response_type,address_id:address_id,type:2,quote_id:quote_id},
		success: function(response) {
		}
	});

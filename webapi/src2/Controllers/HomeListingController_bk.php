...
class HomeListingController
{
	...
	public function newArrivals(Request $request, Response $response, $args){
		...
		$getNewArrivalList = $this->webshop_obj->getNewArrivalList($this->shopcode,$this->shopid,$this->customer_type_id,$limit,'',$lang_code);
	...
	public function featuredProducts(Request $request, Response $response, $args){
		...
		$getNewArrivalList_count = $this->webshop_obj->getNewArrivalList_product_count($this->shopcode,$this->shopid,$this->customer_type_id,$options,$ids,$lang_code); 
		...
		$getNewArrivalList = $this->webshop_obj->getNewArrivalList($this->shopcode,$this->shopid,$this->customer_type_id,$limit,$page,$options,$ids,$lang_code);
	...
	public function testvar($name){
				return json_encode($name.$this->isEuropeShop);

	}
	
	

	public function featuredProductsPage(Request $request, Response $response, $args) {
		//echo "working"; exit();
		$data = $request->getParsedBody();
		extract($data);
		$error = '';

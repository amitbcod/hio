...
//			 echo 'Insert in UST failed. Error: '. $this->dbl->dbl_conn->getLastError();
	...
	function getShopsForBTwoBOrders_opt($shopcode,$shopid,$order_id){
		...
		*/
		...
		$query = "SELECT p.shop_id,oi.order_id FROM $shop_db.sales_order_items as oi  INNER JOIN $shop_db.products as p ON oi.product_id = p.id LEFT JOIN $shop_db.products_inventory as pi ON oi.product_id = pi.product_id WHERE p.product_inv_type IN ('dropship') AND  oi.order_id IN (?) AND p.shop_product_id > 0 group by p.shop_id";

...
class DbProductFeature
{
	...
	public function productListing($categoryid = '', $options = '', $gender = array(), $subscription = array(), $price_range = array(), $variant_id_arr = array(), $variant_attr_value_arr = array(), $attribute_arr = array(), $search_term = '', $page = '', $page_size = '', $categoryIdsarr = array())
	{
		$where_gender = '';
		...
		// $date_sp = time(); //strtotime(date('Y-m-d'));
		...
		$date = time(); //strtotime(date('d-m-Y'));
		//echo $date;
		...
		if ($categoryid != '') {
		...
		} else if (!empty($categoryIdsarr) || is_array($categoryIdsarr)) {
			$categoryIdsarr_val = "'" . implode("','", $categoryIdsarr) . "'";
			$query = "SELECT $distinct $prod_select_list $popular_count $lang_select_data 
							$price_sorting $special_price_sorting 
					FROM products as prod 
					$lang_query 
					INNER JOIN products_category 
						ON prod.id=products_category.product_id 
						AND prod.status=1 
						AND prod.remove_flag=0  
					$where_variant $where_attribute $popular 
					WHERE  
						( $where_search prod.launch_date <= ? 
							OR products_category.category_ids IN ($categoryIdsarr_val) ) 
						AND prod.product_type IN ('simple','configurable','bundle') 
						$where_gender $where_price 
						AND prod.status=? 
						AND prod.approval_status=1 
						AND prod.remove_flag=? 
						$inv_check_where 
					GROUP BY prod.id $order_by $limit ";
		} else {
			$lang_select_data = '';
			// $price_sorting = '';
			// $special_price_sorting = '';
			...
			$query = "SELECT $prod_select_list $popular_count $lang_select_data $price_sorting $special_price_sorting FROM products as prod $lang_query $popular WHERE $where_search prod.launch_date <= ? AND 
				(prod.product_type IN ('simple','configurable','bundle')) AND prod.status=? AND prod.approval_status=1  AND prod.remove_flag=? $order_by  $limit ";
	...
	public function productListingCount($categoryid = '', $options = '', $gender = array(), $price_range = array(), $variant_id_arr = array(), $variant_attr_value_arr = array(), $attribute_arr = array(), $search_term = '', $categoryIdsarr = array(), $page = '', $page_size = '', $language_arr = array())
	{
		$where_gender = '';
		if (!empty($gender)) {
			$str = implode("|", $gender);
		...
		$param = array($date, 1, 0);
		// echo "<pre>";
		// 		print_r($categoryIdsarr);exit;
		if ($categoryid != '') {
			// echo 'hi1';exit;

			$query = "SELECT count(*) as total_row FROM products as prod INNER JOIN products_category ON prod.id=products_category.product_id AND FIND_IN_SET($categoryid,products_category.category_ids) $where_variant $where_attribute $popular WHERE prod.launch_date <= ? AND (prod.product_type IN ('simple','configurable','bundle')) $where_gender $where_price AND prod.status=? AND prod.approval_status=1 AND prod.remove_flag=? $inv_check_where GROUP BY prod.id $order_by  ";
		} else if (!empty($categoryIdsarr) || is_array($categoryIdsarr)) {
			// echo 'hi2';
			...
			$query = "SELECT count(*) as total_row 
					...
					WHERE 
						...
						AND prod.remove_flag=? 
						$inv_check_where 
					GROUP BY prod.id $order_by";
		} else {
			// echo 'hi3';exit;

			$query = "SELECT  count(*) as total_row FROM products as prod $where_variant $where_attribute $popular WHERE $where_search prod.launch_date <= ? AND (prod.product_type IN ('simple','configurable','bundle')) $where_gender $where_price AND prod.status=? AND prod.approval_status=1 AND prod.remove_flag=? $inv_check_where GROUP BY prod.id $order_by   ";
		}
		// echo $order_by;exit;
	...
	public function productDetailsnew(array $product_ids)
	{
		...
		WHERE p.parent_id IN ($product_ids_string) AND p.remove_flag= 0 AND p.status= 1";
        // print_r($query);die;
	...
	public function configurableProductNew($product_id)
	{
		$param = array($product_id);
		$query = "SELECT products.* FROM products WHERE products.parent_id = ? ";
	...
	public function configurableProductForMultipleProductsGift($product_ids)
	{
		$product_ids_string = implode(',', $product_ids);
		$query = "SELECT products.* FROM products WHERE products.parent_id IN ($product_ids_string) ";
	...
	public function variantConfigProduct($ConfigProductId)
	{
		$param = array($ConfigProductId);
		$query = "SELECT eav_attr.id, eav_attr.attr_code, eav_attr.attr_name FROM products_variants  as pv_m INNER JOIN eav_attributes as eav_attr ON eav_attr.id=pv_m.attr_id WHERE pv_m.product_id=?";
		$variant_prod = $this->dbl->dbl_conn->rawQuery($query, $param);
		if ($this->dbl->dbl_conn->getLastErrno() === 0) {
			if ($this->dbl->dbl_conn->count > 0) {
				return $variant_prod;
	...
	{
		// $query = "SELECT DISTINCT pv.attr_value, eav_attr_opt.attr_options_name, eav_attr_opt.position FROM products_variants as pv INNER JOIN eav_attributes_options as eav_attr_opt ON eav_attr_opt.id = pv.attr_value WHERE pv.parent_id=$product_id AND pv.attr_id=$variant_id ";
	...
	public function productImageNewQuery($product_id)
	{

		$param = array($product_id);
		$query = "SELECT base_image FROM products WHERE id=$product_id";
	...
	public function ConfigproductVariantOptionsInstockNewQuery($product_id, $variant_id, $Confproduct_id, $childProductsNotStock)
	{
		// $query = "SELECT DISTINCT pv.attr_value, eav_attr_opt.attr_options_name, eav_attr_opt.position FROM products_variants as pv INNER JOIN eav_attributes_options as eav_attr_opt ON eav_attr_opt.id = pv.attr_value WHERE pv.parent_id=$product_id AND pv.attr_id=$variant_id ";
		// if($childProductsNotStock){
		// 	$query .= " AND pv.product_id NOT IN ($childProductsNotStock)";
		...
		$param = array($product_id, $variant_id);
		$query = "SELECT DISTINCT eav_attr_opt.position,pv.attr_value, eav_attr_opt.attr_options_name, pv.product_id, prod.gift_id, prod.sub_issues, prod.webshop_price, gift.name as gift_master_name FROM products_variants as pv INNER JOIN eav_attributes_options as eav_attr_opt ON eav_attr_opt.id = pv.attr_value INNER JOIN products as prod On pv.product_id = prod.id 
		LEFT JOIN gift_master as gift On prod.gift_id = gift.id WHERE pv.parent_id=$product_id AND pv.product_id=$Confproduct_id AND pv.attr_id=$variant_id ";
		if ($childProductsNotStock) {
			$query .= " AND pv.product_id NOT IN ($childProductsNotStock)";
		}
		$query .= " ORDER BY eav_attr_opt.position ASC";
	...
	public function getmediaGalleryByVariants($product_id, $child_prod_id)
	{
		// $shop_db =  DB_NAME_SHOP_PRE . $shopcode; // constant variable
		// $main_db = DB_NAME; //Constant variable
		...
		$query = "SELECT m_gallery.* FROM products_media_gallery as m_gallery WHERE m_gallery.product_id = ? AND m_gallery.child_id = ? order by  m_gallery.is_base_image,m_gallery.image_position  ASC";
		...
		if ($this->dbl->dbl_conn->getLastErrno() === 0) {
			if ($this->dbl->dbl_conn->count > 0) {
				...
				$query1 = "SELECT m_gallery.* FROM products_media_gallery as m_gallery WHERE m_gallery.product_id = ? AND m_gallery.child_id IS NULL  order by m_gallery.is_base_image,m_gallery.image_position ASC";
	...
	public function getproductDetailsByShopCodeNew($product_id)
	{
		$param = [$product_id];
		$query = "SELECT products.* FROM products WHERE products.id = ? ";

		$product_detail = $this->dbl->dbl_conn->rawQuery($query, $param);
	...
	public function checkInventorySource($product_id,  $seller_shopcode = '')
	{
		$Result = array();
		$source = '';
		// $shop_db =  DB_NAME_SHOP_PRE . $shopcode; // constant variable
			...
			if ($this->dbl->dbl_conn->count > 0) {
				...
				if (($product_inv_type == 'dropship')  || ($product_inv_type == 'virtual' && $inventory['qty'] <= 0)) {
					...
					if ($shop_product_id > 0) {
						...
						$query1 = "SELECT products_inventory.* FROM products_inventory WHERE $seller_db.products_inventory.product_id = ?";
	...
	public function checkProductCountByVariantOption($attr_id, $variant_value_ids, $categoryid, $customer_type_id, $search_term = '', $filter_type = '', $featured_ids = '')
	{
		...
		if ($categoryid != '') {
			...
			INNER JOIN products_category as pc ON pc.product_id=pv.parent_id
			INNER JOIN  products as p ON p.id = pv.parent_id
			WHERE  pv.attr_id = ? AND pv.attr_value  IN (" . implode(',', $variant_value_ids) . ")  AND pc.category_ids=?  AND p.status=1 AND p.remove_flag=0  AND p.launch_date <= ? /*AND ((FIND_IN_SET($customer_type_id,p.customer_type_ids)) OR (p.customer_type_ids='0') $sub_query)*/
			...
			AND p.launch_date <= ? AND ((FIND_IN_SET($customer_type_id,p.customer_type_ids)) OR (p.customer_type_ids='0') $sub_query) $badge_query
			GROUP BY pv.attr_value
			HAVING `count` > 0";
			...
			AND p.launch_date <= ? AND ((FIND_IN_SET($customer_type_id,p.customer_type_ids)) OR (p.customer_type_ids='0') $sub_query) $where_featured
			GROUP BY pv.attr_value
			HAVING `count` > 0";
		} else {
			$param = array($attr_id, $date);
			$query = "SELECT DISTINCT(pv.attr_value) as attr_value, COUNT(*) as count FROM products_variants as pv
			INNER JOIN  products as p ON p.id = pv.parent_id
			WHERE  pv.attr_id = ? AND pv.attr_value IN (" . implode(',', $variant_value_ids) . ") AND p.status=1 AND p.remove_flag=0  AND p.launch_date <= ? AND ((FIND_IN_SET($customer_type_id,p.customer_type_ids)) OR (p.customer_type_ids='0') $sub_query) $where_search
			GROUP BY pv.attr_value
			HAVING `count` > 0";
		}
		// echo $query;
	...
	public function getGiftMaster($giftId)
{
    ...
    $query = "SELECT gift.name FROM gift_master AS gift WHERE gift.id = ?";
    $gift_details = $this->dbl->dbl_conn->rawQueryOne($query, [$giftId]);

    // Check if there is an SQL error
    if ($this->dbl->dbl_conn->getLastErrno() !== 0) {
    ...
    if (!empty($gift_details)) {
        return $gift_details; // Returns the gift record (array)
    }
	...
	public function getCategoryIds($search_term)
	{
		$query = "SELECT * FROM `category` where cat_name LIKE '%$search_term%';";
		$result = $this->dbl->dbl_conn->rawQuery($query);
	...
	//  new
	public function getProductArray($new_product_id)
	{
		// $param = array($new_product_id);
		// print_r($new_product_id);
		// $id_array = explode(',', $new_product_id);

		// $id_string = implode(',', $new_product_id);

		$quoted_ids = "'" . implode("','", $new_product_id) . "'";
		// print_r("SELECT * FROM products WHERE id in($quoted_ids)");
		// die;

		$blocks = $this->dbl->dbl_conn->rawQuery("SELECT * FROM products WHERE id in($quoted_ids) limit 5 ");
		// return $blocks;
		// die;
		// $resultArr = ($blocks !== false) ? [$blocks] : [];
		if ($this->dbl->dbl_conn->getLastErrno() === 0) {
	...
	public function getRegionalProductArray($new_product_id)
	{
		// $param = array($new_product_id);
		// print_r($new_product_id);
		// $id_array = explode(',', $new_product_id);
	...
	public function getRegionalProductAttributeArray($variant_id, $attr_value)
	{
		// print_r($variant_id);
		// print_r($attr_value);
		$param = array($variant_id, $attr_value);
		$blocks = $this->dbl->dbl_conn->rawQuery("SELECT * FROM products_attributes WHERE attr_id = ? and attr_value = ?", $param);
		// print_r($blocks);
		// return $blocks;die;
		// $resultArr = ($blocks !== false) ? [$blocks] : [];
	...
	public function configurableProductForMultipleProductsHindi($new_product_id)
	{
		$product_ids_string = implode(',', $new_product_id);
		$query = "SELECT products.* FROM products WHERE products.parent_id IN ($product_ids_string) ";
	...
	public function configurableProductForMultipleProductsRegional($new_product_id)
	{
		$product_ids_string = implode(',', $new_product_id);
		$query = "SELECT products.* FROM products WHERE products.parent_id IN ($product_ids_string) ";
	...
	public function getHindiMagazineList($limit = '', $product_id = '', $lang_code = '', $flag_rr = '', $page = '', $options = '', $badge = '', $gender = array(), $price_range = array(), $variant_id_arr = array(), $variant_attr_value_arr = array(), $attribute_arr = array())
	{
	...
	public function getRegionalMagazineList($limit = '', $product_id = '', $lang_code = '', $flag_rr = '', $page = '', $options = '', $badge = '', $gender = array(), $price_range = array(), $variant_id_arr = array(), $variant_attr_value_arr = array(), $attribute_arr = array())
	{
	...
	public function configurableRegionalProductForMultipleProducts($new_product_id)
	{
		$product_ids_string = implode(',', $new_product_id);
	...
	public function getProductAttributeArray($variant_id, $attr_value)
	{
		// print_r($variant_id);
		// print_r($attr_value);
		$param = array($variant_id, $attr_value);
	...
	public function getSpecialPricesForMultipleHindiProducts(array $product_ids)
	{
		// $date = strtotime(date('Y-m-d'));
		$date  = time();
		$product_ids_string = implode(',', $product_ids);
	...
	public function getSpecialPricesForMultipleRegionalProducts(array $product_ids)
	{
		// $date = strtotime(date('Y-m-d'));
		$date  = time();
		$product_ids_string = implode(',', $product_ids);
	...
	public function getRegionalProductsBlock($identifier)
	{
	...
	public function getAvailableInventoryForMultipleRegionalProducts($product_ids)
	{
		$product_ids_string = implode(',', $product_ids);
		$query = "SELECT products_inventory.* FROM products_inventory WHERE products_inventory.product_id IN ($product_ids_string)";
	...
	public function getRegionalAvailableInventory($product_id, $Product = null, $inventory = null)
	{
	...
	public function getOptionsByVariantIdForMultipleRegional($attr_ids)
	{
		...
		$query = "SELECT id,attr_id,attr_options_name FROM eav_attributes_options WHERE attr_id IN ($attr_ids_string)  AND  status=1 and attr_options_name not in('Hindi','English and Hindi','English')  ORDER BY eav_attributes_options.position ASC";
		//  echo $query;
	...
	public function getRegionalproductDetailsById($product_id)
	{
		return  $this->getRegionalproductDetailsByShopCode($product_id);
	}

	public function getRegionalproductDetailsByShopCode($product_id)
	{
		$param = [$product_id];
		$query = "SELECT products.* FROM products WHERE products.id = ? ";
	...
	public function getProductAttributeCountArray($variant_id, $attr_value)
	{
		// print_r($variant_id);
		// print_r($attr_value);
		$param = array($variant_id, $attr_value);
		$blocks = $this->dbl->dbl_conn->rawQuery("SELECT count(*) as count FROM products_attributes WHERE attr_id = ? and attr_value = ? ", $param);
		// return $blocks;die;
		// $resultArr = ($blocks !== false) ? [$blocks] : [];

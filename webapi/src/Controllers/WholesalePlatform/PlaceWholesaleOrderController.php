...
class PlaceWholesaleOrderController
{
    ...
    private function createOrderItems($data, $products): void
    {
        foreach ($data['order_items'] as $order_item) {
            ...
            $item_price = round($order_item['price'] * (1 + ($data['vat_percentage'] / 100)), 2);
    ...
    private function createSalesOrder($data, $customer, $currencies)
    {
        ...
        foreach ($data['order_items'] as $order_item) {
            $item_price = round($order_item['price'] * (1 + ($data['vat_percentage'] / 100)),2);
            $item_vat = $item_price - $order_item['price'];
            $base_subtotal += $order_item['quantity'] * $item_price;
            $total_qty_ordered += $order_item['quantity'];
            $base_tax_amount += ($order_item['quantity'] * $item_vat);// Order tax (vat)
        }
        ...
        $subtotal = $base_subtotal + $shipping_amount; // Order total INCL tax EX shipping
        $base_grand_total = $base_subtotal +  $shipping_amount; // Order Total (incl tax & shipping)

...
class ProductFiltersController
{
	...
	public function productNavFilters(Request $request, Response $response, $args)
	{
		...
		$product_array = [];
		$attribut_array = $this->getAttributeArray($categoryid, $search_term, $filter_type, $featured_ids);

		$variantArray = $this->getVariantArray($categoryid, $search_term, $filter_type, $featured_ids);
		...
		foreach ($data as $new_data) {
			...
			$product_array['product_list_count'][$new_data['attr_value']] = $this->webshop_obj->getProductAttributeCountArray($variant_id, $attr_value);
			// $product_array['product_list_count'] = 
		}
	...
	private function getVariantArray(string $categoryid, string $search_term, string $filter_type, string $featured_ids): array
	{
		$variantArray = [];
		$variantProduct = $this->webshop_obj->getFiltersVariantMaster($categoryid);
	...
	private function getAttributeArray(string $categoryid, string $search_term, string $filter_type, string $featured_ids): array
	{
		$attribut_array = [];
		$attributData = $this->webshop_obj->getFiltersAttributeMasterOfShop();

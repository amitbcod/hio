...
$app->post('/webshop/contact_us', function (Request $request, Response $response) {
	...
	$webshopName = $Common_obj->getWebShopName();
	...
	if ($post_lcode != '') {
		...
		$LangCodeName = $webshop_obj->getCodeByLanguageName($post_lcode);
	...
	$DynamicVars = array($name, $email, $orderflag, $orderID, $content);

	$site_logo = '';
	$CommonVars = array($site_logo, $webshop_name);

	$EmailToAdmin = $webshop_obj->get_custom_variable('contact_us_email');

	$EmailTo = $EmailToAdmin['value'] ?? 'no-reply@shopinshop.com';
	// $EmailTo = 'snehals@bcod.co.in';

	$emailSendStatusFlag = $webshop_obj->get_email_code_status("contact_us");


	if ($emailSendStatusFlag == 1) {
		// $get_email = $webshop_obj->sendCommonHTMLEmail($EmailTo, "contact_us", $TempVars, $DynamicVars, '', '', $CommonVars, $lang_code);

		$get_email = $webshop_obj->sendCommonHTMLEmailTest($EmailTo, "contact_us", $TempVars, $DynamicVars, '', '', $CommonVars, $lang_code);
	} else {
		$get_email == false;
	}
	// if ($get_email == false) {
	// 	abort('Mail Not Sent');
	// }
	$order_increment_id = (isset($order_increment_id) ?  $order_increment_id : '');
	$post_lcode = (isset($post_lcode) ?  $post_lcode : '');
	$order_flag = (isset($order_flag) ?  $order_flag : '');
	$mobileno = (isset($mobileno) ?  $mobileno : '');

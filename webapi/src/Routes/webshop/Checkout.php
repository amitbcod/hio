...
$app->post('/webshop/remove_quote', function (Request $request, Response $response, $args) {
	...
	if ($quote_id == '') {
		...
		$cartData = $ch_obj->removeQuote($quote_id, $customer_id);
...
$app->post('/webshop/update_quote_customer_id', function (Request $request, Response $response, $args) {
	...
	if ($quote_id == '' || $customer_id == '' || $session_id == '') {
		...
		$cartData = $ch_obj->updateQuoteCustomer($quote_id, $session_id, $customer_id, $checkout_method);
...
$app->post('/webshop/place_order', function (Request $request, Response $response, $args) {
	...
	if (empty($quote_id) || empty($address_options) || empty($payment_method)) {
		...
		$lis_default_language = (isset($lis_default_language) && $lis_default_language != '') ? $lis_default_language : 0;
		...
		if ($address_options != 'new') {
			...
			$AddressInfo = $ch_obj->get_customer_address_by_id($customer_id, $address_options);
		...
		if ($billing_address_options != 'new') {
			...
			$BillingAddressInfo = $ch_obj->get_customer_address_by_id($customer_id, $billing_address_options);
		...
		if ($address_options == 'new' && (empty($shipping_first_name) || empty($shipping_last_name) || empty($shipping_mobile_no) || empty($shipping_address)  || empty($shipping_country) || empty($shipping_pincode) || empty($shipping_city))) {
			...
			$Shippingcompany_name = $Shippingcompany_name ?? '';
			...
			$QuoteData = $ch_obj->getQuoteDataById($quote_id);
			...
			if ($QuoteData == false) {
				...
				$QuotePayment = $ch_obj->getQuotePaymentDataById($quote_id);
				...
				$payment_gateway_charges = $QuoteData['payment_gateway_charges'];
				...
				if ($order_id == false) {
					...
					$QuoteItems = $ch_obj->get_sales_quote_items($quote_id);
					...
					if (isset($QuoteItems) && count($QuoteItems) > 0) {
						...
						foreach ($QuoteItems as $item) {
							...
							$ch_obj->add_to_sales_order_item($order_id, $item['product_type'], $item['product_inv_type'], $item['product_id'], $item['product_name'], $item['product_code'], $item['qty_ordered'], $item['sku'], $item['barcode'], $item['price'], $item['total_price'], $item['created_by'], $item['parent_product_id'], $item['product_variants'], $item['estimate_delivery_time'], $item['applied_rule_ids'], $item['tax_percent'], $item['tax_amount'], $item['discount_amount'], $item['discount_percent'], $item['total_discount_amount'], $item['is_fragile_flag'], $coupon_code, $zumba_shop_flag, $item['bundle_child_details'], $ship_method_id, $shipping_tax_percent);
					...
					if ($address_options == 'new') {
						...
						if ($AddressInfo != false) {
							...
							$Shippingcompany_name = ((isset($AddressInfo['company_name']) && $AddressInfo['company_name'] != '') ? $AddressInfo['company_name'] : '');
					...
					$ch_obj->add_to_sales_order_address($order_id, 1, $billing_first_name, $billing_last_name, $billing_address_options, $billing_mobile_no, $billing_address, $billing_address_1, $billing_city, $billing_state, $billing_country, $billing_pincode, $billing_save_in_address_book, $company_name, $vat_no, $same_as_billing);
					...
					$ch_obj->add_to_sales_order_address($order_id, 2, $shipping_first_name, $shipping_last_name, $address_options, $shipping_mobile_no, $shipping_address, $shipping_address_1, $shipping_city, $shipping_state, $shipping_country, $shipping_pincode, $save_in_address_book, $Shippingcompany_name, '', $same_as_billing);
					...
					$ch_obj->update_vatDetails_in_sales_order($order_id, $company_name, $vat_no);
					...
					if ($QuotePayment == false) {
						...
						$ch_obj->add_to_sales_order_payment($order_id, $payment_method_id, $payment_method, $payment_method_name, $payment_type, $currency_code);
					...
					// $FbcData=$ch_obj->getFbcUserIdByShopId();
					...
					// 	$ch_obj->add_to_order_log($type,$order_id,$fbc_user_id);
					...
					$orderData = $ch_obj->getOrderDataById($order_id);



					$increment_id = $orderData['increment_id'];

					$voucher_code   = isset($orderData['voucher_code']) ? $orderData['voucher_code'] : '';
					$voucher_amount = isset($orderData['voucher_amount']) ? (float)$orderData['voucher_amount'] : '';
					$order_barcode = isset($orderData['order_barcode']) ? (float)$orderData['order_barcode'] : '';
	...
	if ($error != '') {
		...
		$message['voucher_code'] = $voucher_code;

		$message['voucher_amount'] = $voucher_amount;

		$message['order_barcode'] = $order_barcode;
...
$app->post('/webshop/check_quote_item_available', function (Request $request, Response $response, $args): Response {
	...
	if (!empty($QuoteItems) && count($QuoteItems) > 0) {
		...
		foreach ($QuoteItems as $item) {
			...
			$max_qty_product = $gbl_feture->get_custom_variable($identifier);
			...
			if ($item['product_type'] == 'conf-simple') {
				...
				$productData = $webshop_obj->getproductDetailsById($item['product_id']);
				...
				$productDataTemp = $webshop_obj->getproductDetailsById($item['product_id']);
				...
				if ($item['product_type'] == 'bundle') {
					...
					foreach ($bundle_child_details as $childDetails) {
						...
						$product_inv = $webshop_obj->getAvailableInventory($childDetails['product_id']);
						...
						$ordered_quatity = $item['qty_ordered'];



						$default_quantity = $childDetails['default_qty'];
	...
	// 	'All products quantity is available..'
...
$app->post('/webshop/set_checkout_method', function (Request $request, Response $response, $args) {
	...
	if ($quote_id == '' || $checkout_method == '') {
		...
		$cartData = $ch_obj->updateCheckoutMethod($quote_id, $checkout_method);
...
$app->post('/webshop/payment_methods_listing', function (Request $request, Response $response, $args) {
	...
	if ($country_code == '') {
		...
		$Result = $ch_obj->getPaymentMethods($country_code);
		...
		if ($Result == false) {
			...
			foreach ($Result as $value) {
				...
				if ($value['payment_type'] == 1) {
					...
					$MainPayInfo = $ch_obj->getMainShopPaymentDetailsById($value['main_payment_id']);
...
$app->post('/webshop/set_checkout_payment_method', function (Request $request, Response $response, $args) {
	...
	if ($quote_id == '' || $payment_id == '' || $payment_type == '') {
		...
		if ($payment_type == 1) {
			...
			$PayInfo = $ch_obj->getWebShopPaymentDetailsById($payment_id);
		...
		$QuotePayExist = $ch_obj->getQuotePaymentById($quote_id);

		// echo "<pre>";

        // print_r($QuotePayExist);die;
		...
		if ($payment_type == 2) {
			...
			$grandTotal = $QuoteData['base_subtotal'] + $QuoteData['shipping_amount'];



			// Adjust grandTotal if coupon_code is not zero

			if (!empty($QuoteData['coupon_code'])) {

				$grandTotal -= $QuoteData['discount_amount'];

			}elseif (!empty($QuoteData['voucher_code'])) {

				$grandTotal -= $QuoteData['voucher_amount'];
			...
			if ($grandTotal >= 5000) {                
				...
				$additional_tax = $grandTotal * 0.02; // 2%

				$grandTotal += $additional_tax;
		...
		$columns = ' grand_total = ?, payment_gateway_charges= ?';



		$where = ' quote_id = ? ';

		$params = array($grandTotal, $additional_tax , $quote_id);

		



		$Row = $common_obj->update_row($table, $columns, $where, $params);
		...
		if ($QuoteData['payment_final_charge'] > 0.00) {
			...
			$columns = ' base_grand_total = ?, grand_total = ?, payment_gateway_charges= ?, payment_charge = ?, payment_tax_percent = ?, payment_tax_amount = ?, payment_final_charge = ?';
			...
			$params = array($baseGrandTotal, $grandTotal, $additional_tax, $payment_charge, $payment_tax_percent, $payment_tax_amount, $payment_final_charge, $quote_id);
		...
		if ($QuotePayExist == false) {
			...
			$Row = $ch_obj->updateQuotePaymentMethod($quote_id, $payment_method_id, $payment_method, $payment_method_name, $payment_type, $gateway_details);
...
$app->post('/webshop/send_order_confirmation_email', function (Request $request, Response $response, $args) {
	...
	if (empty($order_id) || empty($currency_code)) {
		...
		$OrderDetail = $ch_obj->getOrderDataById($order_id);
		...
		$OrderItems = $ch_obj->getOrderItems($order_id);
		...
		if ($OrderDetail != false) {
			...
			$ch_obj->updateOrderStatus($order_id, 0);  // set order status to 0 = Processing
			...
			$payment_gateway_charges = number_format($OrderDetail['payment_gateway_charges'], 2);
			...
			if ($PaymentInfo['payment_method_id'] == 8) {

				$CHEQUE_PAYMENT_NOTE = '<tr><h3><strong>You can directly deposit the cheque/DD or transfer payment online at any branch of ICICI Bank in India</strong></h3><tr>';

				$CHEQUE_PAYMENT_DETAIL = '<tr>

				<td height="40" style="font-family:times new roman;font-size:16px;font-weight:bold">Following are the bank details.</td>
			  ...
			  <tr>
				...
				<td><table width="100%" border="0" cellspacing="0" cellpadding="5" style="border:1px solid #000;font-family:times new roman;font-size:16px;font-weight:bold">

				  <tbody><tr>

					<td width="100%" style="border-bottom:1px solid #000">ICICI BANK</td>            

				  </tr>

				  <tr>

					<td valign="top" style="line-height:20px"><p>A/c No: 123205000772</p>

					  <p>Account Name: WHUSO Ecommerce Solutions Pvt. Ltd.</p>

					  <p>IFSC Code: ICIC0001232</p>

					  <p>Branch Name: Saibabanagar, Borivali West Branch</p></td>           
			...
			if ($customer_id == 0) {
				...
				$customer_details = $webshop_obj->getCustomerDetailById($customer_id);



				$Customer_Type =  $webshop_obj->getCustomerType($customer_details['customer_type_id']);
			...
			$TemplateContentData = $email_obj->getWebShopEmailTemplateByCode($identifier, $lang_code);
			...
			if ($TemplateContentData == false) {
				...
				$FromEmail = $email_obj->get_custom_variable('admin_email');
				...
				$Sales_Email_Details = $email_obj->get_custom_variable('sales_admin_email');
				...
				if ($FromEmail == false) {
					...
					$store_admin_email = 'no-reply@indiamags.com';
				...
				$StoreMobile = $email_obj->get_custom_variable('store_mobile');
				...
				if ($FromEmail == false) {
					...
					$store_phone = 'no-reply@indiamags.com';
				...
				if (isset($OrderItems) && count($OrderItems) > 0) {
					...
					foreach ($OrderItems as $item) {
						...
						$prod_id=$item['parent_product_id'];

						$cat_id = $ch_obj->getCategoryId($prod_id);

						$new_cat_id= $cat_id['category_ids'];

						// echo "<pre>";print_r($new_cat_id);die();

						if ($new_cat_id == 80) {

							if ($PaymentInfo['payment_method_id'] == 2){

								// Ensure $site_url and $encoded_id are defined

								$digital_cat_access = 'To access your digital magazine <a href="' . $site_url . 'e-magazine/' . $encoded_id . '" target="_blank">(click here)</a>. You will receive a password OTP in a separate email.';
						...
						$gift_type = '<p style="font-weight: 500;font-size: 13px;line-height: 15px;color: #787878;">Gift: ' . $gift_name . '</p>';
							...
							<table cellpadding="0" cellspacing="0" border="0" style="width:100%;padding:0;margin:0;border-top:1px dashed #c3ced4;border-bottom:1px dashed #c3ced4">
								...
								<tbody>
									...
									<tr style="padding-bottom:5px">
										...
										<td colspan="3" align="right" style="padding:3px 9px;font-family:Verdana,Arial;font-weight:normal;border-collapse:collapse;vertical-align:top;margin:0">' . $item_price_txt . '</td>
									...
									<tr style="padding-bottom:5px">
										...
										<td colspan="3" align="right" style="padding:3px 9px;font-family:Verdana,Arial;font-weight:normal;border-collapse:collapse;vertical-align:top;margin:0">' . $Taxes_txt . '</td>
									...
									<tr style="padding-bottom:5px">
										...
										<td colspan="3" align="right" style="padding:3px 9px;font-family:Verdana,Arial;font-weight:normal;border-collapse:collapse;vertical-align:top;margin:0">' . $shipping_txt . '</td>
								...
								$order_items .= '
									...
									<tr>
										...
										<td colspan="3" align="right" style="padding:3px 9px;font-family:Verdana,Arial;font-weight:normal;border-collapse:collapse;vertical-align:top;margin:0">' . $Subtotal_txt . '</td>
								...
								$order_items .= $voucher_html . $payment_html;

								if ($PaymentInfo['payment_method_id'] == 2){

									if ($grand_total_final >= 5000) {

										$order_items .= '

											<tr style="padding-bottom:5px">

												<td colspan="3" align="right" style="padding:3px 9px;font-family:Verdana,Arial;font-weight:normal;border-collapse:collapse;vertical-align:top;margin:0">Payment gateway charges</td>

												<td align="right" style="padding:3px 9px;font-family:Verdana,Arial;font-weight:normal;border-collapse:collapse;vertical-align:top;margin:0">

													<span style="font-family:&quot;Helvetica Neue&quot;,Verdana,Arial,sans-serif">' . $payment_gateway_charges . '</span>
				...
				$invoice_link = 'To download your invoice , go to your My Orders <a href="' . $site_url . 'customer/my-orders" target="_blank">(click here)</a>.';

				$TempVars = array('##LOGIN_URL##', '##STORE_EMAIL##', '##STORE_PHONE##', '##INCREMENT_ID##', '##ORDER_DATE##', '##CUSTOMER_NOTE##', '##ORDER_ITEMS##', '##BILLING_ADDRESS##', '##SHIPPING_ADDRESS##', '##PAYMENT_METHOD##', '##WEBSHOPNAME##', '##CUSTOMER_GROUP##', '##CHEQUE_PAYMENT_NOTE##', '##CHEQUE_PAYMENT_DETAIL##', '##DIGITAL_CAT_ACCESS##', '##INVOICE##');

				$DynamicVars = array($login_url, $store_sales_admin_email, $store_phone, $increment_id, $order_date, $customer_note, $order_items, $billing_address, $shipping_address, $payment_method_name, $websopname, $customer_group_name, $CHEQUE_PAYMENT_NOTE, $CHEQUE_PAYMENT_DETAIL,$digital_cat_access,$invoice_link);
				...
				$emailSendStatusFlag = $email_obj->get_email_code_status($identifier);
				...
				if ($emailSendStatusFlag == 1) {
					...
					$email_send =  $email_obj->sendCommonHTMLEmail($EmailTo, $identifier, $TempVars, $DynamicVars, $increment_id, '', $CommonVars, $lang_code);
					...
					if ($email_send == false) {
						...
						$ch_obj->updateOrderEmailSentNotification($order_id);
				...
				// 		$admin_email_send1 = $email_obj->sendCommonHTMLEmail('anu@bcod.co.in',$identifier,$TempVars,$DynamicVars,$increment_id,'',$CommonVars,$lang_code);



				// 		$admin_email_send2 = $email_obj->sendCommonHTMLEmail('accounts@whuso.in',$identifier,$TempVars,$DynamicVars,$increment_id,'',$CommonVars,$lang_code);



				// 		$admin_email_send3 = $email_obj->sendCommonHTMLEmail('heeral@whuso.in',$identifier,$TempVars,$DynamicVars,$increment_id,'',$CommonVars,$lang_code);



				// 		$admin_email_send4 = $email_obj->sendCommonHTMLEmail('usha@bcod.co.in',$identifier,$TempVars,$DynamicVars,$increment_id,'',$CommonVars,$lang_code);
				...
				$SalesAdminEmail = $email_obj->get_custom_variable('sales_admin_email');
				...
				if ($SalesAdminEmail == false) {
					...
					if ($emailSendStatusFlag == 1) {
						...
						$admin_email_send =  $email_obj->sendCommonHTMLEmail($sales_admin_email, $identifier, $TempVars, $DynamicVars, $increment_id, '', $CommonVars, $lang_code);

					}

				}

				$sales_head_email = "heeral@whuso.in";

				if ($emailSendStatusFlag == 1) {



					$admin_head_email_send =  $email_obj->sendCommonHTMLEmail($sales_head_email, $identifier, $TempVars, $DynamicVars, $increment_id, '', $CommonVars, $lang_code);
...
// 		$OrderDetail = $ch_obj->getOrderDataByIdForOrderEmail($order_id);
...
// 		$new_order_id = $OrderDetail['order_id'];

// 		$OrderItems = $ch_obj->getOrderItems($new_order_id);
...
// 			$billing_address = $ch_obj->getFormattedOrderAddressById($new_order_id, 1);
...
// 			$shipping_address = $ch_obj->getFormattedOrderAddressById($new_order_id, 2);
...
// 			$PaymentInfo = $ch_obj->getOrderPaymentDataById($new_order_id);
...
// 								$digital_cat_access = 'To access your digital magazine <a href="https://indiamags.com/e-magazine/' . $encoded_id . '" target="_blank">(click here)</a>. You will receive a password OTP in a separate email.';
...
// 										<p style="font-family:Verdana,Arial;font-weight:bold;margin:0 0 5px 0;color:#636363;font-style:normal;text-transform:uppercase;line-height:1.4;font-size:14px;float:left;width:100%;display:block">' . $item['product_name'] . '</p> ' . $variant_type . ' ' . $gift_type . '
...
// 				$TempVars = array('##LOGIN_URL##', '##STORE_EMAIL##', '##STORE_PHONE##', '##INCREMENT_ID##', '##ORDER_DATE##', '##CUSTOMER_NOTE##', '##ORDER_ITEMS##', '##BILLING_ADDRESS##', '##SHIPPING_ADDRESS##', '##PAYMENT_METHOD##', '##WEBSHOPNAME##', '##CUSTOMER_GROUP##', '##CHEQUE_PAYMENT_NOTE##', '##CHEQUE_PAYMENT_DETAIL##', '##DIGITAL_CAT_ACCESS##');
...
// 				$DynamicVars = array($login_url, $store_sales_admin_email, $store_phone, $increment_id, $order_date, $customer_note, $order_items, $billing_address, $shipping_address, $payment_method_name, $websopname, $customer_group_name, $CHEQUE_PAYMENT_NOTE, $CHEQUE_PAYMENT_DETAIL, $digital_cat_access);
...
// $app->post('/webshop/send_order_confirmation_email_test', function (Request $request, Response $response, $args) {
...
// 				$TempVars = array('##LOGIN_URL##', '##STORE_EMAIL##', '##STORE_PHONE##', '##INCREMENT_ID##', '##ORDER_DATE##', '##CUSTOMER_NOTE##', '##ORDER_ITEMS##', '##BILLING_ADDRESS##', '##SHIPPING_ADDRESS##', '##PAYMENT_METHOD##', '##WEBSHOPNAME##', '##CUSTOMER_GROUP##', '##CHEQUE_PAYMENT_NOTE##', '##CHEQUE_PAYMENT_DETAIL##');
...
// 				$DynamicVars = array($login_url, $store_sales_admin_email, $store_phone, $increment_id, $order_date, $customer_note, $order_items, $billing_address, $shipping_address, $payment_method_name, $websopname, $customer_group_name, $CHEQUE_PAYMENT_NOTE, $CHEQUE_PAYMENT_DETAIL);
...
// 					$email_obj->sendCommonHTMLEmail_test('rajesh@bcod.co.in', $identifier, $TempVars, $DynamicVars, $increment_id, '', $CommonVars, $lang_code);
...
$app->post('/webshop/generate_b2b_order_for_webshop', function (Request $request, Response $response, $args) {
	...
	if (empty($order_id)) {
		...
		$OtherShops = $ch_obj->getShopsForBTwoBOrders($order_id);
		...
		if ($OtherShops != false) {
			...
			foreach ($OtherShops as $shop) {
				...
				$Products = $ch_obj->getProductsDropShipAndVirtualWithQtyZero($order_id, $seller_shop_id);
				...
				if ($Products != false) {
					...
					foreach ($Products as $value) {
						...
						// $shop_order_items[$seller_shop_id]['shipping_amount'][] += $value['shipping_amount'];
						...
						// $checkSpecialPrice = $ch_obj->getSpecialPricingB2b($seller_shop_id, $value['product_id']);



						$checkSpecialPrice = $product_obj->getSpecialPrices($value['product_id']);
						...
						$SellerProductData = $product_obj->getproductDetailsByShopCode($shop_product_id);

						// print_R($value['qty_ordered']);

						// die();

						$shipping_amount += ($value['qty_ordered'] * $value['shipping_amount']) ;
				...
				// if (isset($SellerProductData['shipping_amount']) && $SellerProductData['shipping_amount'] > 0) {

				// 	$shipping_amount += $SellerProductData['shipping_amount'];
				...
				$shop_order_items[$seller_shop_id]['shipping_amount'] = $shipping_amount;

			}

				// print_r($shipping_amount);
			...
			// $SellerFbcUserData = $ch_obj->getPublisherIdByShopId($seller_shop_id);
			...
			$SellerFbcData = $ch_obj->getFbcUserShopDataByShopId();
			...
			// $seller_shipping_amount = $OrderDetail['shipping_amount'];
			...
			$payment_gateway_charges = $OrderDetail['payment_gateway_charges'];



			$seller_fbc_user_id = $SellerFbcData['id'];
			...
			// $webshop_name = $SellerFbcData['website_domain_name'];
			...
			if ($SellerFbcData != false) {
				...
				if (isset($shop_order_items) && count($shop_order_items) > 0) {
					...
					foreach ($shop_order_items as $seller_shop_id => $value) {
						...
						$created_by = $SellerFbcUserData['created_by'];
						...
						// Convert the array of item IDs into a comma-separated string

						$item_ids = implode(',', $order_item_ids);

						// if (isset($shop_order_items) && count($shop_order_items) > 1) {

						

						// 	$seller_shipping_amount += $value['shipping_amount'];
						...
						// print_r($value['shipping_amount']);die;

						// Check if there are multiple items (i.e., if `order_item_ids` is an array with more than one element)
						...
						$seller_commision_percent = $SellerFbcUserData['commision_percent'];

						$whuso_income = (($seller_commision_percent  / 100) * $grand_total);
						...
						$b2b_order_id = $ch_obj->create_b2b_order_for_webshop($order_id, $customer_firstname, $customer_lastname, $customer_is_guest, $subtotal, $grand_total, $base_subtotal, $base_grand_total, $total_qty_ordered, $discount_percent, $discount_amount, $whuso_income, $created_by, $seller_shop_id, $payment_gateway_charges, $discount_amout_special_price, $seller_shipping_amount);
						...
						if ($b2b_order_id != false) {
							...
							$OrderItems = $ch_obj->getB2BOrderItemsByIds($order_id, $item_ids);
							...
							if ($OrderItems != false) {
								...
								foreach ($OrderItems as $item) {
									...
									$BuyerProductData = $product_obj->getproductDetailsByShopCode($item['product_id']);
									...
									if ($BuyerProductData != false) {
										...
										if ($SellerProductData != false) {
											...
											//	$checkSpecialPrice = $ch_obj->getSpecialPricingB2b($seller_shop_id, $shop_product_id);

											$checkSpecialPrice = $product_obj->getSpecialPrices($item['product_id']);
...
print_r([
    ...
    'seller_shop_id_repeat'         => $seller_shop_id,

    'special_price_flag'            => $special_price_flag,

    'special_price_original_price'  => $special_price_original_price,
...
echo '</pre>';*/
									...
									$ch_obj->add_to_b2b_order_item($seller_shop_id, $b2b_order_id, $item['product_type'], $product_id, $product_name, $product_code, $item['qty_ordered'], $sku, $barcode, $item_price, $item_total_price, $item['created_by'], $parent_product_id, $item['product_variants'], '', $item['is_fragile_flag'], $tax_percent, $tax_amount, 0, $seller_shop_id, $special_price_flag, $special_price_original_price);



//echo "-------------------------------------------------------after add item called";
									...
									$CheckProduct = false; //$product_obj->checkInventorySource($item['product_id']);
								...
								if ($total_tax_amount > 0) {
									...
									$ch_obj->update_b2b_order_for_webshop($b2b_order_id, $seller_shop_id, $total_tax_amount, $total_tax_amount, $grandtotal_updated, $grandtotal_updated);
							...
							$OrderDetail = $ch_obj->getB2BOrderDataByOrderId($b2b_order_id);



							$OrderItems = $ch_obj->getB2BOrderItemsByOrderId($seller_shop_id, $b2b_order_id);
							...
							if ($OrderDetail != false) {
								...
								if ($TemplateContentData == false) {
									...
									// 	$email_send =  $email_obj->sendCommonHTMLEmail($EmailTo, $identifier, $TempVars, $DynamicVars, $increment_id, $webshop_name, $CommonVars, $lang_code);
									...
									// 		$ch_obj->updateB2BOrderEmailSentNotification($order_id);
...
$app->post('/webshop/update_order_status', function (Request $request, Response $response, $args) {
	...
	if ($order_id == '' || $status == '') {
		...
		$cartData = $ch_obj->updateOrderStatus($order_id, $status);
...
$app->post('/webshop/cc_avenue_initialize', function (Request $request, Response $response, $args) {
	...
	$access_code = 'AVUC05CH96AM83CUMA'; //Shared by CCAVENUES
...
$app->post('/webshop/get_price_split_for_publisher', function (Request $request, Response $response, $args) {
	...
	if (empty($order_id)) {
		...
		if ($OtherShops != false) {
			...
			foreach ($OtherShops as $shop) {
				...
				$publisher_details = $ch_obj->getPublisherIdByShopId($seller_shop_id);

				// print_r($publisher_details);

				$shop_order_items[$seller_shop_id]['publisher_details'] = $publisher_details;
			...
			if ($SellerFbcData != false) {
				...
				if (isset($shop_order_items) && count($shop_order_items) > 0) {
					...
					foreach ($shop_order_items as $seller_shop_id => $value) {
						...
						// $b2b_order_id = $ch_obj->create_b2b_order_for_webshop($order_id, $customer_firstname, $customer_lastname, $customer_is_guest, $subtotal, $grand_total, $base_subtotal, $base_grand_total, $total_qty_ordered, $discount_percent, $discount_amount, $discount_amout_special_price, $seller_shop_id);
						...
						if ($OrderItems != false) {
							...
							foreach ($OrderItems as $item) {
								...
								// $ch_obj->add_to_b2b_order_item($seller_shop_id, $b2b_order_id, $item['product_type'], $product_id, $product_name, $product_code, $item['qty_ordered'], $sku, $barcode, $item_price, $item_total_price, $item['created_by'], $parent_product_id, $item['product_variants'], '', $tax_percent, $tax_amount, 0, $seller_shop_id, $special_price_flag, $special_price_original_price);
	...
	if ($error != '') {
		...
		$message['publisher_item_details'] = $shop_order_items;
...
$app->post('/webshop/abundantCartDetails', function (Request $request, Response $response, $args) {
	...
	if (empty($quote_id)) {
		...
		$OrderData = $ch_obj->abundantCartDetails($quote_id);
		...
		if (!empty($OrderData)) {
			...
			$session_id = $OrderData[0]['session_id'];

		} else {

			$error = "No data found for the given quote ID";
...
$app->post('/webshop/get_order_payment', function (Request $request, Response $response, $args) {
	...
	if (empty($order_id)) {
		...
		$OrderData = $ch_obj->get_order_payment($order_id);
		...
		// print_r($OrderData);die;
			...
			// $data['OrderData'] = $OrderData[0]['order_id'];

...
$app->get('/webshop/my_orders_listing/{shopcode}/{shopid}/{customer_id}/{limit}/{offset}', function (Request $request, Response $response, $args){
...
$app->get('/webshop/my_orders_listing_new/{shopcode}/{shopid}/{customer_id}/{limit}/{offset}', function (Request $request, Response $response, $args){
	...
	if($shopcode =='' || $shopid=='' ||  $customer_id =='')
	{
		...
		$custom_variables_data = $webshop_obj->get_custom_variables($shopcode);
		...
		if($OrderData!=false){
			...
			$OrderItemsMultidata=$order_obj->getOrderItemsMultiids($shopcode,$shopid,$order_ids);
			$this->order_products = array_group($OrderItemsMultidata, 'order_id');
			$b2bShopsIds=$ch_obj->getShopsForBTwoBOrdersMultiple($shopcode,$shopid,$order_ids);
			...
			// return order data
			$OrderReturnOrdersMultidata=$order_obj->getReturnOrdersDataMultiple($shopcode,$shopid,$order_ids);
			if($OrderReturnOrdersMultidata!=false){
				...
				$OrderReturnItemsMultidata=$order_obj->getReturnOrdersItemsDataMulti($shopcode,$shopid,$order_ids,$return_order_ids);

...
$app->post('/webshop/product_detail', function (Request $request, Response $response, $args){
	...
	if($productData['product_type'] === 'simple'){
		...
		}elseif($productData['product_inv_type'] === 'dropship'){
			$seller_shopcode = 'shop'.$productData['imported_from']; // changed by al  shop_id to imported_from
		...
		$bundleProducts = $webshop_obj->bundleProduct($shopcode,$shopid,$productData['id']);
		...
		$BundleProductChildDetails= $webshop_obj->getproductStatusById($shopcode,$bundle_child_ids);
		...
		foreach ($bundleProducts as $value) {
			...
			if($value['product_type'] == 'configurable'){
				...
				if($product_variants != ''){
					...
					$VariantOptionsSelected = $webshop_obj->getVariantOptionsSelected($shopcode,$main_prd_id,$attr_id,$attr_value,$selected_variant_count);
				...
				$productDetails = $wishlist_obj->getproductDetailsById($shopcode,$shopid,$main_prd_id,$lang_code);
				...
				$variants_arr = array();
				$variantProduct = $webshop_obj->configProductVariant($shopcode,$shopid,$main_prd_id);
					if($variantProduct!=false){

						foreach ($variantProduct as $variant) {
							if(empty($prod_selected_ids)){
								$variantOption = $webshop_obj->productVariantOptionsInstockNewQuery($shopcode,$shopid,$main_prd_id,$variant['id'],'',$prelaunch);
							}else{
								$variantOption = $webshop_obj->productVariantOptionsFilterByProductIds($shopcode,$shopid,$main_prd_id,$prod_selected_ids,$variant['id']);
							}
				...
				$product_inv =  $webshop_obj->getAvailableInventory($value['product_id'],$shopcode);
				if(is_numeric($product_inv['available_qty']) && $product_inv['available_qty'] > 0) {
					...
					if($estcount == 0) {
						...
						$delay_warehouse_timing = (isset($delay_warehouse_time['value']) && $delay_warehouse_time['value'] != '') ? $delay_warehouse_time['value'] : 0;
						$estimate_delivery_time = $ProDelTime1 + $delay_warehouse_timing;
						$arr['estimate_delivery_time'] = $estimate_delivery_time;
				...
				if($productDetails['product_type'] == 'configurable'){
					...
					$VariantInfo=$cart_obj->get_product_variant_details($shopcode,$value['product_parent_id'],$value['product_id']);
					...
					$arr['variants'] = $product_variants_str;
				}else{
					$arr['variants'] = '';
				}
		...
		if(($qty > 0 || $prelaunch == 'yes' ) && $bundle_child_NotInv == 0){ // check stock
			$productData['stock_status'] = 'Instock';
...
//
//		$productData['variant_master']= $variantMaster ?? [];
		...
		if(!empty($buyInventoryIds)){
			$buyInventory = $webshop_obj->getAvailableInventoryForMultipleProducts($buyInventoryIds, $shopcode);
			$buyInventory = array_combine(array_column($buyInventory, 'product_id'), $buyInventory);
		}

		$allSpecialPrices = $webshop_obj->getSpecialPricesForMultipleProducts($shopcode, array_column($configProduct, 'id') , $customer_type_id);
	...
	// print_r($AttributesWithOptions) ;die();
...
$app->post('/webshop/get_conf_simprod_by_variants', function (Request $request, Response $response, $args){
	...
	if($shopcode=='' || $shopid=='' || $product_id=='' || $total_variant=='' || $selected_variant=='')
	{
		...
		$product_exists = $webshop_obj->productExists($shopcode, $product_id);
		...
		if($total_variant>0){
			...
			if(isset($conf_simple_product_id)  && $conf_simple_product_id!='' && $conf_simple_product_id>0){
				...
				if($specialPriceArr!=false){
					if(isset($vat_percent_session) && $vat_percent_session !='') {
						$ConfigSimpleResponse['special_price'] = round(($specialPriceArr['special_price'] * $vat_percent_session/100)+$specialPriceArr['special_price'],2);
					}else{
...
$app->post('/webshop/getBundleChildValidateQty', function (Request $request, Response $response, $args){
		...
		if(isset($conf_simple_array) && !empty($conf_simple_array)){
			foreach($conf_simple_array as $confSimple){
				...
				$bundleProductsdata =  $webshop_obj->bundleProductById($shopcode,$shopid,$confSimple['bundle_child_id']);
				...
				$product_inv = $webshop_obj->getAvailableInventory($confSimple['conf_simple_pid'],$shopcode);

				$total_qty = ($qty * $bundleProductsdata['default_qty']);
				$total_other_qty = 0;
				if(isset($quote_id) && $quote_id!=''){
					$total_other_qty = $webshop_obj->GetTotalQuoteAddedInventoryExceptCurrentId($shopcode,$shopid,$quote_id,$bundleProductsdata['bundle_product_id'],$confSimple['conf_simple_pid'],$parent_id);
				}
		...
		if(isset($bundle_products_ids) && $bundle_products_ids !=''){
			...
			foreach($bundle_products_ids as $id){
				$bundleProductsdata = $webshop_obj->bundleProductItemByIdWithInventory($shopcode,$shopid,$main_bundle_id,$id);

				$total_qty = ($qty * $bundleProductsdata['default_qty']);
				$total_other_qty = 0;
				if(isset($quote_id) && $quote_id!=''){
					$total_other_qty = $webshop_obj->GetTotalQuoteAddedInventoryExceptCurrentId($shopcode,$shopid,$quote_id,$bundleProductsdata['bundle_product_id'],$bundleProductsdata['product_id'],$bundleProductsdata['product_parent_id']);
				}
				$total_qty = $total_qty + $total_other_qty;
...
$app->post('/webshop/getSimpleValidateQty', function (Request $request, Response $response, $args){
	...
	if($shopcode =='' || $shopid=='' || $qty=='')
	{
		...
		$total_bundle_qty = 0;
		if(isset($quote_id) && $quote_id!=''){
			$total_bundle_qty = $webshop_obj->GetTotalQuoteAddedInventoryExceptCurrentId($shopcode,$shopid,$quote_id,'',$product_id,$parent_product_id);

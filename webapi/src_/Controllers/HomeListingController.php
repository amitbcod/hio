...
class HomeListingController
{
	...
	public function newArrivals(Request $request, Response $response, $args){
		...
		if($this->shopid==1){
			...
			if(isset($webshop_base_color_status) && !empty($webshop_base_color_status)){
				...
				if(isset($this->base_color_status) && !empty($this->base_color_status) && $this->base_color_status=="yes"){
					...
					$multiple_variants_products = $this->product_obj->configProductVariantProduct($this->shopcode, $this->shopid, $product_ids);
					if(!empty($multiple_variants_products)){
						$multiple_variant_id = array_column($multiple_variants_products, 'id');
						$variantOption = $this->product_obj->productMultipleVariantOptions($this->shopcode, $this->shopid, $product_ids,$multiple_variant_id);
		...
		$wishlistData = [];
		if( $this->shopid == 1 && isset($customer_login_id) && $customer_login_id > 0) {
	...
	public function newArrivals_page(Request $request, Response $response, $args){
		...
		$getNewArrivalList_count = $this->webshop_obj->getNewArrivalList_product_count($this->shopcode,$this->shopid,$this->customer_type_id,'',$lang_code,'new_arrivals',$options,$badge,$gender, $price_range, $variant_id_arr, $variant_attr_value_arr, $attribute_arr);
		...
		$getNewArrivalList = $this->webshop_obj->getNewArrivalList($this->shopcode,$this->shopid,$this->customer_type_id,$limit,'',$lang_code,'new_arrivals',$page,$options,$badge,$gender, $price_range, $variant_id_arr, $variant_attr_value_arr, $attribute_arr);
	...
	public function featuredProducts_page(Request $request, Response $response, $args){
		...
		$getNewArrivalList_count = $this->webshop_obj->getNewArrivalList_product_count($this->shopcode,$this->shopid,$this->customer_type_id,$ids,$lang_code,'',$options,'',$gender, $price_range, $variant_id_arr, $variant_attr_value_arr, $attribute_arr);
		if($getNewArrivalList_count!=false){
			$getNewArrivalList_count=$getNewArrivalList_count;
		}else{
			$getNewArrivalList_count=0;
		...
		$getNewArrivalList = $this->webshop_obj->getNewArrivalList($this->shopcode,$this->shopid,$this->customer_type_id,$limit,$ids,$lang_code,'',$page,$options,'',$gender, $price_range, $variant_id_arr, $variant_attr_value_arr, $attribute_arr);
	...
	private function advancedProductStrategyEurope($value): array
	{
		...
		if (count($configProduct) > 0) {
			...
			foreach ($configProduct as $conf) {
				...
				if(isset($this->base_color_status) && $this->base_color_status == 'yes' && !empty($this->getAvailableQuantity($conf)) && $this->getAvailableQuantity($conf) > 0){

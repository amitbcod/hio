...
Class DbCommonFeature{
	...
	public function getThemeByShopcode()
	 ...
	 public function CustomerDetailsByEmailId($email)
	 {
	...
	public function commonLoginFunction($loginID,$first_name,$last_name,$email,$customer_type_id,$quote_id='',$remember='',$vat_percent_session='',$currency_name='',$currency_code_session='',$currency_conversion_rate='',$default_currency_flag='',$currency_symbol='')
  	{
		$cart_obj = new DbCart();
		...
		if($quote_id!=''){
			$new_Quote_id = $quote_id;
				...
				if(!empty($quote_ids_previous)){
					...
					$prelauch_product_list = $this->getPrelauchProductList();
					$customer_active_prelauch = $this->getCustomerPrelaunchStatus($loginID);
					...
					if(!empty($quote_item)) {
						...
						if($item['product_type'] != 'bundle'){
							...
							$getProductsInfo = $this->getProductsDetails($item['product_id'],$isPrelaunch);

							if(!empty($getProductsInfo)){

								if($getProductsInfo['product_type'] == 'conf-simple'){
									$getConfigProductsInfo = $this->getProductsDetails($getProductsInfo['parent_id'],$isPrelaunch);
								...
								if(in_array($customer_type_id, $customer_type_ids) || $getProductsInfo['customer_type_ids'] == 0) {
									...
									$specialPriceArr = $webshop_obj->getSpecialPrices($getProductsInfo['id'],$customer_type_id);
									...
									if((!empty($getProductsInfo) && $product_inv['available_qty'] > 0) || $isPrelaunch == 1){
										...
										if($checkItemExist==false){
											...
											$cart_item_added=$cart_obj->add_to_sales_quote_item($quote_id,$product_type,$product_inv_type,$cart_product_id,$product_name,$product_code,$quantity,$sku,$barcode,$price,$total_price,$created_by,$parent_product_id,$product_variants_str,$tax_percent,$tax_amount,$isPrelaunch);
											...
											*/
											...
											if($isPrelaunch == 1){
												...
												$specialPirceArr = $webshop_obj->getSpecialPrices($getProductsInfo['id'],$customer_type_id);
			...
			$this->updateCartDataBasedOnCustomerType($customer_type_id,$quote_id,$vat_percent_session);
				...
				if(isset($quote_id) && $quote_id != '')
				{
						...
						$updateCurrency = $cart_obj->updateQuoteCurrency($quote_id,$currency_name,$currency_code_session,$currency_conversion_rate,$default_currency_flag,$currency_symbol);
				}
		}else{
				...
				if(!empty($quote_ids_previous)){
							...
							if(!empty($getProductsInfo)){
								...
								if(in_array($customer_type_id, $customer_type_ids) || $getProductsInfo['customer_type_ids'] == 0) {
									...
									if((!empty($getProductsInfo) && $product_inv['available_qty'] > 0) || $isPrelaunch == 1){
										...
										$cart_item_added=$cart_obj->add_to_sales_quote_item($new_Quote_id,$product_type,$product_inv_type,$cart_product_id,$product_name,$product_code,$quantity,$sku,$barcode,$price,$total_price,$created_by,$parent_product_id,$product_variants_str,$tax_percent,$tax_amount,$isPrelaunch);
					...
					{
						$ch_obj = new DbCheckout();
						$ch_obj->updateTaxAndShippingCharges($shopid,$new_Quote_id);
						$cart_obj->UpateQuoteTotal($new_Quote_id);

						$updateCurrency = $cart_obj->updateQuoteCurrency($new_Quote_id,$currency_name,$currency_code_session,$currency_conversion_rate,$default_currency_flag,$currency_symbol);
					}
...
// ,'access_prelanch_product'=>$customer_active_prelauch['access_prelanch_product'],'allow_catlog_builder'=>$allow_catlog_builder['allow_catlog_builder']
  	...
  	{
		$query = "SELECT sales_quote_items.sku, sales_quote_items.product_id, sales_quote_items.parent_product_id,sales_quote_items.shop_id,sales_quote_items.prelaunch,sales_quote_items.product_type, SUM(sales_quote_items.qty_ordered) as qty_ordered FROM sales_quote_items WHERE quote_id in ($quote_ids) group by product_id";
	  ...
	  public function getProductsDetails($product_id,$isPrelaunch){

		$date = strtotime(date('d-m-Y'));

		$sub_query = '';
		if($isPrelaunch != 1){
			$sub_query = "AND products.status = 1 AND products.launch_date <= $date ";
		}
		$query = "SELECT products.* FROM products WHERE products.id = $product_id AND products.remove_flag = 0 $sub_query";
	...
	public function checkQuoteItemDataExistById($quote_id,$product_id)
	{
	   if(isset($parent_product_id) && $parent_product_id!=''){
		   $sql =  "SELECT * FROM sales_quote_items where `product_id` = '$product_id'  AND quote_id = $quote_id ";
			...
			$sql =  "SELECT * FROM sales_quote_items where   AND `product_id` = '$product_id' AND quote_id = $quote_id ";
	   }
	...
	public function getCustomerCatlogBuilderStatus($shopcode,$loginID){
  	...
  	public function updateCartDataBasedOnCustomerType($customer_type_id,$quote_id,$vat_percent='')
  	{
		...
		if($OrderItems!=false){
			foreach ($OrderItems as $value) {
				...
				if(in_array($customer_type_id, $customer_type_ids) || $product_datails['customer_type_ids'] == 0) {
					...
					$specialPriceArr = $product_obj->getSpecialPrices($value['product_id'],$customer_type_id);
	...
	public function getDelayWarehouseTime($shopcode)
	{
		...
		if(isset($results[$shopcode])){
			return $results[$shopcode];
		}

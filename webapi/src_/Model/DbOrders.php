...
Class DbOrders{
	...
	public function __construct()
	{
	 ...
	 public function getReturnOrdersDataMultiple($shopcode,$shopid,$order_ids)
	 {
		$shop_db =  DB_NAME_SHOP_PRE.$shopcode; // constant variable
		$main_db = DB_NAME; //Constant variable
		$order_ids_string = implode(',',$order_ids);
		$sql =  "SELECT * FROM $shop_db.sales_order_return where `order_id` IN ($order_ids_string) AND status NOT IN (0,1) order by created_at DESC";
		 $row  = $this->dbl->dbl_conn->rawQuery($sql);
		if ($this->dbl->dbl_conn->getLastErrno() === 0){
			if ($this->dbl->dbl_conn->count > 0){
				return $row;
	 ...
	 public function getReturnOrdersItemsDataMulti($shopcode,$shopid,$order_ids,$return_order_ids)
	 {
		$shop_db =  DB_NAME_SHOP_PRE.$shopcode; // constant variable
		$main_db = DB_NAME; //Constant variable
		...
		$sql =  "SELECT * FROM $shop_db.sales_order_return_items where `order_id` IN ($order_ids_string)  AND return_order_id IN ($return_order_ids_string) ";
		$row  = $this->dbl->dbl_conn->rawQuery($sql);
		if ($this->dbl->dbl_conn->getLastErrno() === 0){
			if ($this->dbl->dbl_conn->count > 0){
				$return_order_items=array();
				$item_order_ids = array_column($row, 'order_id');
				$OrderItemsMultidata=$this->getOrderItemsMultiidsReturnOrder($shopcode,$shopid,$item_order_ids);
	...
	public function get_tracking_data_for_b2b($shopcode,$order_ids)
	{
	 ...
	 public function getOrderItems($shopcode,$shopid, $order_id)
	 {
		...
		$main_db = DB_NAME; //Constant variable
		 ...
		 $sql =  "SELECT * FROM $shop_db.sales_order_items where `order_id` = '$order_id' AND `product_type`!= 'bundle' order by item_id ASC";
	 ...
	 public function getOrderItemsMultiids($shopcode,$shopid, $order_ids)
	 {

		$shop_db =  DB_NAME_SHOP_PRE.$shopcode; // constant variable
		$main_db = DB_NAME; //Constant variable
		$order_ids_string = implode(',',$order_ids);
		$sql =  "SELECT item_id,shop_id,order_id,product_id,parent_product_id,parent_item_id,product_type,product_name,product_code,product_variants,estimate_delivery_time,qty_ordered,qty_scanned,price,total_price,tax_amount,discount_amount,total_discount_amount,can_be_returned FROM $shop_db.sales_order_items where `order_id` IN ($order_ids_string) AND `product_type`!= 'bundle' order by item_id ASC";
		$Result  = $this->dbl->dbl_conn->rawQuery($sql);
	...
	function decrementAvailableQty($shopcode,$product_id,$qty_ordered){
		...
		public function getB2BOrdersForWebshop($shopcode,$order_id,$webshop_shop_id)
	 {
		 $shop_db =  DB_NAME_SHOP_PRE.$shopcode; // constant variable
		$main_db = DB_NAME; //Constant variable


		 $sql =  "SELECT order_id,increment_id,created_at,grand_total,status,is_split FROM $shop_db.b2b_orders where `webshop_order_id` = '$order_id' AND `shop_id` = '$webshop_shop_id' AND status!=7 order by created_at DESC";
		 $row  = $this->dbl->dbl_conn->rawQuery($sql);
		if ($this->dbl->dbl_conn->getLastErrno() === 0){
			if ($this->dbl->dbl_conn->count > 0){
				return $row;
 	...
 	public function getOrdersItemsDetailsByID($shopcode,$order_id)
	{
		...
		$param = array($order_id);
		$sql = "SELECT soi.* FROM $shop_db.sales_order_items as soi WHERE soi.order_id = ? AND soi.product_type != 'bundle' ORDER BY soi.item_id ASC";

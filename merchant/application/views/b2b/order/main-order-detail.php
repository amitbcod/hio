					...
					<?php if (empty($this->session->userdata('userPermission')) || in_array('b2webshop/orders/write', $this->session->userdata('userPermission'))) { ?>
						<div class="save-discard-btn pad-bottom-20">
							...
							?>
								...
								<!-- <button  name="cancel_order_btn" class="btn btn-primary" data-toggle="modal" id="cancel_order_btn" data-id="<?php echo $OrderData->order_id; ?>" value="<?php //echo $order->order_id; 
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response !== 'error') {
                    ...
                    swal("Error", "Could not load Mark as Failed popup", "error");
                }
            }
        });
    }

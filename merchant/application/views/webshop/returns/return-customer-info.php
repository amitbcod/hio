<div class="barcode-qty-box row order-details-sec-top">
	<div class="col-sm-6 order-id">
		<p><span> Order Number :</span> <?php echo $ReturnOrderData->return_order_increment_id; ?> (Order <a href="<?=base_url()."webshop/shipped-order/detail/".$OrderData->order_id?>" target="_blank">#<?=$OrderData->increment_id?></a>)</p>
	...
	<div class="col-sm-6 order-id">
		...
		<?php if(isset($current_tab) && $current_tab=='refund-request-order'){ ?>
			<div class="col-sm-4 order-id return-request">
				...
				<div class="col-sm-4 order-id return-request">
					<p><span>Payment Mode </span> <?php echo $payment_method_name; ?> </p>
				</div>			
...
<?php if($ReturnOrderData->refund_payment_mode==2){ ?>
					...
					<div class="col-sm-4 order-id return-request">
					<p><span>Bank Name </span> <?php echo (isset($ReturnOrderData->bank_name) && $ReturnOrderData->bank_name!='')?$ReturnOrderData->bank_name:'-'; ?> </p>
				</div>
				<div class="col-sm-4 order-id return-request">
				...
				<div class="col-sm-4 order-id return-request">
					<p><span>Bank Branch </span> <?php echo (isset($ReturnOrderData->bank_branch) && $ReturnOrderData->bank_branch!='')?$ReturnOrderData->bank_branch:'-'; ?> </p>
				</div>

				<div class="col-sm-4 order-id return-request">
					<p><span>Account Holder Name</span> <?php echo (isset($ReturnOrderData->acc_holder_name) && $ReturnOrderData->acc_holder_name!='')?$ReturnOrderData->acc_holder_name:'-'; ?> </p>
				</div>
...
<?php }else if($ReturnOrderData->refund_payment_mode==1){ ?>
	<div class="col-sm-10 pad-zero barcode-order-details-entry">
				...
				<?php } ?>
<?php } else if($ReturnOrderData->refund_payment_mode==3 || $ReturnOrderData->refund_payment_mode==4){  ?>
	...
	<div class="col-sm-10 pad-zero barcode-order-details-entry">
			...
			<p>
			<?php
			if(isset($ReturnOrderData->refund_coupon_code) && $ReturnOrderData->refund_coupon_code!=''){
				if(isset($returnStripeData)){
					echo "Online Refund (Online Refund:". $currency_code.' '.number_format($returnStripeData->order_grandtotal_approved,2). ", Store Credit:". $currency_code.' '.number_format($returnStripeData->order_grandtotal_approved_voucher,2).")";
				}else{
					echo "Online Refund";
				}
			}else{
	...
	<?php } ?>
		...
		<div class="col-sm-4 order-id return-request">
			<p><span>Original Order QTY </span> <?php echo $OrderData->total_qty_ordered; ?> </p>
			<?php
				$qtyData=$this->ReturnOrderModel->getReturnOdrTotalApprovedQTY($ReturnOrderData->return_order_id);
			?>
			<p><span>Return/Refund QTY </span> <?php echo $qtyData->qty_return_approved; ?> </p>
		</div>

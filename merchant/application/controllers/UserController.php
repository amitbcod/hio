			...
			if(empty($_POST['inputFirstName']) || empty($_POST['inputLastName']) || empty($_POST['inputTradeName']) || empty($_POST['inputShopUrl']) || empty($_POST['inputBrnNumber'])  || empty($_POST['inputEmail']) || empty($_POST['inputPassword']) || empty($_POST['inputConfirmPassword'])) {
				...
				if ($merchant_id) {
					$this->db->insert('merchant_addon_purchases', [
					 ...
					 $this->db->insert('publisher_subscriptions', [
						'publisher_id' => $merchant_id,          // Assuming publisher_id = merchant_id
						'plan_id'      => 2,                     // Default plan (change as needed)
						'status'       => 'active',
						'start_date'   => date('Y-m-d H:i:s'),
						'end_date'     => date('Y-m-d H:i:s', strtotime('+1 year')), // 1-year subscription
						'ip'           => $_SERVER['REMOTE_ADDR'],
						'created_at'   => date('Y-m-d H:i:s'),
						'updated_at'   => NULL
					]);
...
exit;*
			...
			foreach ($requiredFields as $field) {
				if (empty($UserdataDetails->$field)) {
					$redirect = base_url("PublisherController/editMerchant/" . $UserDetails->id);
					break;
				}
			}
   ...
   public function logout()

	{
		...
		if ($LoginToken && $LoginID) {
			...
			$updatetime = ['logout_time' => time()];

			$this->db->where(['id' => $LoginID, 'sessionid' => $LoginToken])

					->update('adminsession', $updatetime);
	...
	public function store() {
        ...
        $existing = $this->db->get_where('publisher', ['email' => $email])->row();
        if ($existing) {
            echo json_encode([
                'status' => 'error',
		...
		$site_logo = '<img src="https://ymstore.whuso.in/uploads/yellow-markets-logo.png" alt="Yellow Markets Commerce" width="248" height="auto">';

		...
		<tbody>
			<?php if (isset($OrderItems) && count($OrderItems) > 0) {
				...
				foreach ($OrderItems as $item) {
					...
					// 	   $item_class='black-row';
					//   }
					$newSubStartDate = date(SIS_DATE_FM, $item->sub_start_date);
					$newSubEndDate = date(SIS_DATE_FM, $item->sub_end_date);
					if ($newSubStartDate == '') {
						$newSubStartDate = '-';
					}
					if ($newSubEndDate == '') {
			...
			?>
					<tr class="<?php echo $item_class; ?>" id="oi-single-<?php echo $item->item_id; ?>">
						...
						?>
							...
							<?php //if(empty($this->session->userdata('userPermission')) || in_array('webshop/orders/write',$this->session->userdata('userPermission'))){ 
							?>
							<button  type="button" class="purple-btn btn-sm" onclick="OpenScanWtihQtyPopup(<?php //echo $OrderData->order_id; 
						...
						<?php //} 
						?><?php //if($current_tab=='order' && $OrderData->status==0 && empty($OrderData->coupon_code) && empty($OrderData->voucher_code) && (empty($OrderPaymentDetail) || $OrderPaymentDetail->payment_method_id==4 || $OrderPaymentDetail->payment_method_id==5)){ 
									?>
						<?php //if(empty($this->session->userdata('userPermission')) || in_array('webshop/orders/write',$this->session->userdata('userPermission'))){ 
						?>
							<button  type="button" class="btn btn-primary change-add-n-btn" style="position: static;right: 0;margin-left: 5px;" onclick="OpenDeletePopup(<?php //echo $OrderData->order_id; 
																																											?>,<?php //echo $item->item_id; 
						...
						<td>
							...
							<button type="button" class="btn btn-primary change-add-n-btn" style="position: static;right: 0;margin-left: 5px;" onclick="OpenVariantPopup(<?php echo $item->item_id; ?>);">
						...
						<!-- <td><?php //echo $item->qty_scanned; 
									?></td> -->
						<?php if ($use_advanced_warehouse->value !== 'yes') : ?>
							<td><?= $item->prod_location ?? '-' ?></td>
						<?php endif; ?>
						<?php if ($current_tab != 'create-shipment' && $use_advanced_warehouse->value !== 'yes') { ?>
							<td><span <?php if ($item->estimate_delivery_time > $product_delivery_duration->value) {
											echo "class='estimate_delivery_time'";
										} ?>> <?php
																																									if ($current_tab == 'split-order') {
																																										// now fixed
...
<script type="text/javascript">
	...
	function OpenVariantPopup(item_id) {
		if (item_id != '') {
			$.ajax({
				url: BASE_URL + "WebshopOrdersController/openVariantPopup",
				type: "POST",
				data: {
					//   order_id:order_id,
					item_id: item_id
	...
	function ConfirmVariants(item_id) {
		...
		// return false;
		$.ajax({
			url: BASE_URL + "WebshopOrdersController/itemwithvar",
			type: "POST",
			dataType: "json",
			data: {
				item_id: item_id,
			...
			success: function(response) {
				...
				// if (obj) {
				// 	$("#FBCUserCommonModal").modal('hide');
				// 	RefreshOrderItems(item_id, current_tab);
				// 	swal({
				// 			title: "Updated",
				// 			text: "Product variant updated",
				// 			type: "success",
				...
				// 		function() {
				// 			// location.href = BASE_URL + "/dashboard";
				// 			location.href = BASE_URL + "UserController/logout";
		...
		// 	$('#conf-var-btn').prop('disabled', true);

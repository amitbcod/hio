...
class UserController extends CI_Controller
{
	...
	public function signUpPostData()
	{
		if (isset($_POST) && $_POST != '' && $_POST['sign-up-btn'] == 'Sign Up') {
			if(empty($_POST['inputName']) || empty($_POST['inputEmail']) || empty($_POST['inputPassword']) || empty($_POST['country']) || empty($_POST['currency']) || empty($_POST['inputTerms']) ) {
				...
				$verifyResponse = file_get_contents_curl('https://www.google.com/recaptcha/api/siteverify?secret='.GC_SECRETE_KEY.'&response='.$_POST['g-recaptcha-response']);
				...
				if($responseData->success) {
					$name = $this->CommonModel->custom_filter_input($_POST['inputName']);
					...
					$userExist = $this->UserModel->getUserByEmail($email);
					if(isset($userExist) && $userExist->fbc_user_id!='')
					{
						echo json_encode(array('flag'=>0, 'msg'=>"User already registered with this email address."));
						exit;
					}

					$Find_UserName = str_replace(" ", "-", trim($name));
					$Find_UserName = preg_replace('/[^A-Za-z0-9-]/', '', $Find_UserName);
					$Find_UserName = strtolower($Find_UserName);
					$Identifier = '';
					$IdentifierSave = 'no';

					$identifierExist = $this->UserModel->checkUserIdentifierExist($Find_UserName);
					// echo "user count";
					// print_r(sizeof($identifierExist));

					if(sizeof($identifierExist) <= 0){
						$IdentifierSave = 'yes';
						$Identifier = $Find_UserName;
					}
					...
					//create user
					$this->db->insert('fbc_users', $insertdata);
					$fbc_user_id= $this->db->insert_id();

					if($IdentifierSave == 'no') {
						$Identifier = $Find_UserName."-".$fbc_user_id;
						$this->UserModel->updateUserIdentifierByUserId($fbc_user_id, $Identifier);
					}

					//$countryData = $this->CommonModel->getCurrencySymbolByCountryCode($country_code);
					...
					//create user
					$this->db->insert('fbc_users_shop', $insertShopdata);
					...
					$verify_url = BASE_URL."verify-email-address/".$encoded_data;
					$this->session->set_flashdata('verify_email_link',$verify_url);
					$templateId ='fbcuser-register-emailverify';
					$to = $email;
					$encoded_id_email = urlencode($to.'-'.$fbc_user_id);

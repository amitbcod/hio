			...
			if (empty($_POST)) {
				...
				if (empty($_POST['product_name']) || empty($_POST['product_code']) ||  empty($_POST['description']) || empty($_POST['highlights'])) {
					...
					if (isset($_FILES["gallery_image"]["name"]) &&  is_array($_FILES["gallery_image"]["name"]) &&  count($_FILES["gallery_image"]["name"]) > 0) {
						...
						// print_r($_FILES["gallery_image"]["name"]);
						// echo "hello";die();
			 ...
			 if(isset($product_data) && count($product_data)>=1){
				foreach($product_data as $index=>$product)
				{
					...
					if($sub_category!='' ){
						...
						if(isset($MainCat) && $MainCat['id']!='')
						{
							$SubCat=$this->CategoryModel->check_child_category_exist($shop_id,$MainCat['id'],1,$sub_category);
			 ...
			 if(isset($headers) && count($headers)>0){
				 foreach($headers as $columns){
					 if (strpos($columns, 'attribute_') !== false) {
						...
						$CheckAttr=$this->EavAttributesModel->check_attr_exist_by_shop_and_attr_code($shop_id,2,$attr_code);
						if(isset($CheckAttr) && $CheckAttr->id!=''){
			 ...
			 $master_variant_key='';
			  ...
			  if(isset($product_data) && count($product_data)>1){
				  foreach($product_data as $index=>$product){
					...
					if(in_array("product_code",$headers)){
					...
					if(in_array("publisher_id",$headers)){
						$key30 = array_search('publisher_id', $headers);
						$publisher_id= $product[$key30];
					}
					if(in_array("pub_com_per_type",$headers)){
						$key31 = array_search('pub_com_per_type', $headers);
						$pub_com_per_type= $product[$key31];
					}
					if(in_array("pub_com_percent",$headers)){
						$key32 = array_search('pub_com_percent', $headers);
						$pub_com_percent= $product[$key32];
					}
					if(in_array("description",$headers)){
						$key8 = array_search('description', $headers);
						$description= $product[$key8];
					}
					if(in_array("highlights",$headers)){
						$key9 = array_search('highlights', $headers);
						$highlights= $product[$key9];
					}
					if(in_array("product_reviews_code",$headers)){
						$key10 = array_search('product_reviews_code', $headers);
						$product_reviews_code= $product[$key10];
					}
					if(in_array("launch_date",$headers)){
						$key11 = array_search('launch_date', $headers);
						$launch_date= $product[$key11];
					}
					if(in_array("estimate_delivery_time",$headers)){
						$key12 = array_search('estimate_delivery_time', $headers);
						$estimate_delivery_time= $product[$key12];
					}

					if(in_array("sku",$headers)){
						$key14 = array_search('sku', $headers);
						$sku= $product[$key14];
					}
					...
					if(in_array("inventory",$headers)){
						$key16 = array_search('inventory', $headers);
						$stock_qty= $product[$key16];
					}

					if(in_array("cost_price",$headers)){
						$key17 = array_search('cost_price', $headers);
						$cost_price= $product[$key17];
					}
					if(in_array("selling_price",$headers)){
						$key18 = array_search('selling_price', $headers);
						$price= $product[$key18];
					}
					if(in_array("tax_percent",$headers)){
						$key19 = array_search('tax_percent', $headers);
						$tax_percent= $product[$key19];
					}
					...
					if(in_array("status",$headers)){
						$key22 = array_search('status', $headers);
						$status= $product[$key22];
					}
					if(in_array("customer_types",$headers)){
					...
					if(in_array("meta_title",$headers)){
						$key24 = array_search('meta_title', $headers);
						$meta_title= $product[$key24];
					}
					if(in_array("meta_keyword",$headers)){
						$key25 = array_search('meta_keyword', $headers);
						$meta_keyword= $product[$key25];
					}
					if(in_array("meta_description",$headers)){
						$key26 = array_search('meta_description', $headers);
						$meta_description= $product[$key26];
					}
					if(in_array("search_keywords",$headers)){
						$key27 = array_search('search_keywords', $headers);
						$search_keywords= $product[$key27];
					}
					if(in_array("promo_reference",$headers)){
						$key28 = array_search('promo_reference', $headers);
						$promo_reference= $product[$key28];
					}
					if(in_array("weight",$headers)){
										...
										if(isset($images) && count($images)>0){
											...
											foreach($images as $ig=>$orig_name){
												...
												if(filter_var($orig_name, FILTER_VALIDATE_URL) !== false)
												{
														...
														$new_file_name=$sku.'_'.time().'_'.$ig.'.'.$extension;
											...
											if(filter_var($orig_name, FILTER_VALIDATE_URL) !== false)
												{
														...
														$thumb_folder=SIS_SERVER_PATH.'/'.$shop_upload_path.PRODUCT_GALLERY_PATH.'thumb/';
														$medium_folder=SIS_SERVER_PATH.'/'.$shop_upload_path.PRODUCT_GALLERY_PATH.'medium/';
														$large_folder=SIS_SERVER_PATH.'/'.$shop_upload_path.PRODUCT_GALLERY_PATH.'large/';
														$original_folder=SIS_SERVER_PATH.'/'.$shop_upload_path.PRODUCT_GALLERY_PATH.'original/';
										...
										foreach($var_list as $row_key=>$attr_code){
											if($product[$row_key]!=''){
												...
												if($VarData){
													...
													$OptionData=$this->EavAttributesModel->check_attributes_options_exist_by_seller($shop_id,$attr_id,$attr_value);
